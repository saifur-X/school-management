...
import { 
  ...
  Activity, Bell, CalendarCheck, Briefcase, UploadCloud, BarChart3, Download, Lock, Megaphone, Sparkles
} from 'lucide-react';
...
export default function Dashboard() {
  ...
  const fetchStudentSpecificData = async (studentId, studentClass) => {
    ...
    setStudentPortalData({ marks: marks || [], tx: tx || [] }); 
    setLoading(false);
  };
  ...
  if (!session && !studentSession && !teacherSession) {
    return <div className="min-h-screen bg-slate-950 flex items-center justify-center text-white"><Loader2 className="animate-spin mr-2"/> Loading Portal...</div>;
  }
    ...
    return (
      <div className="min-h-screen bg-slate-950 text-slate-100 font-sans p-6 md:p-10">
        <div className="max-w-6xl mx-auto space-y-8">
          <div className="flex justify-between items-center bg-slate-900/80 border border-slate-800 p-6 rounded-3xl shadow-xl backdrop-blur-xl">
            <div className="flex items-center gap-4">
              {school.logo_url && <img src={school.logo_url} alt="Logo" className="w-14 h-14 rounded-full bg-white object-contain p-1 border-2 border-blue-500 shadow-[0_0_20px_rgba(59,130,246,0.3)]" />}
            ...
            <button onClick={handleLogout} className="bg-rose-500/10 text-rose-400 hover:bg-rose-500/20 px-5 py-2.5 rounded-xl font-bold flex items-center gap-2 transition btn-glow-rose"><LogOut size={16}/> Logout</button>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="bg-slate-900/80 border border-slate-800 p-6 rounded-3xl flex flex-col justify-center backdrop-blur-md"><p className="text-xs text-slate-400 uppercase tracking-widest font-bold">Total Fees Paid</p><h3 className="text-4xl font-black text-emerald-400 mt-2">₹{totalPaid}</h3></div>
            <div className="bg-slate-900/80 border border-slate-800 p-6 rounded-3xl flex flex-col justify-center backdrop-blur-md"><p className="text-xs text-slate-400 uppercase tracking-widest font-bold">Current Dues (Arrears)</p><h3 className="text-4xl font-black text-rose-400 mt-2">₹{totalDue}</h3></div>
          </div>

          <div className="bg-slate-900/60 border border-slate-800 p-6 rounded-3xl shadow-xl space-y-4 backdrop-blur-xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {studentNotices.length > 0 ? studentNotices.map((n) => (
                <div key={n.id} className="bg-slate-950/80 p-5 rounded-2xl border border-slate-800 flex flex-col justify-between space-y-3 hover:border-amber-500/40 transition">
                  <div>
                ...
                <div className="col-span-2 text-center p-8 text-slate-500 border border-dashed border-slate-800 rounded-2xl">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div className="bg-slate-900/60 border border-slate-800 p-6 rounded-3xl shadow-xl backdrop-blur-xl">
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-slate-950 text-slate-100 font-sans relative overflow-hidden">
      
      {/* গ্লোবাল মডার্ন নিওন গ্লো সিএসএস ও প্রিন্ট স্টাইল */}
      <style dangerouslySetInnerHTML={{ __html: `
        .scrollbar-hide::-webkit-scrollbar { display: none; }
        .scrollbar-hide { -ms-overflow-style: none; scrollbar-width: none; }

        /* নিওন গ্লো বাটন ইফেক্টস */
        .btn-glow-blue {
          box-shadow: 0 0 15px -1px rgba(59, 130, 246, 0.45);
          transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1);
        }
        .btn-glow-blue:hover {
          box-shadow: 0 0 26px 2px rgba(59, 130, 246, 0.75);
          transform: translateY(-2px);
        }
        .btn-glow-emerald {
          box-shadow: 0 0 15px -1px rgba(16, 185, 129, 0.45);
          transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1);
        }
        .btn-glow-emerald:hover {
          box-shadow: 0 0 26px 2px rgba(16, 185, 129, 0.75);
          transform: translateY(-2px);
        }
        .btn-glow-amber {
          box-shadow: 0 0 15px -1px rgba(245, 158, 11, 0.45);
          transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1);
        }
        .btn-glow-amber:hover {
          box-shadow: 0 0 26px 2px rgba(245, 158, 11, 0.75);
          transform: translateY(-2px);
        }
        .btn-glow-purple {
          box-shadow: 0 0 15px -1px rgba(168, 85, 247, 0.45);
          transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1);
        }
        .btn-glow-purple:hover {
          box-shadow: 0 0 26px 2px rgba(168, 85, 247, 0.75);
          transform: translateY(-2px);
        }
        .btn-glow-rose {
          box-shadow: 0 0 15px -1px rgba(244, 63, 94, 0.45);
          transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1);
        }
        .btn-glow-rose:hover {
          box-shadow: 0 0 26px 2px rgba(244, 63, 94, 0.75);
          transform: translateY(-2px);
        }
      ...
      {toast.show && (
        <div className={`fixed bottom-6 right-6 z-[100] px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-3 font-bold text-sm transform transition-all animate-bounce-in ${toast.type === 'error' ? 'bg-rose-600 text-white shadow-rose-600/30' : 'bg-emerald-600 text-white shadow-emerald-600/30'}`}>
      ...
      {isAdmissionModalOpen && (
        <div className="fixed inset-0 bg-slate-950/90 backdrop-blur-sm z-[60] flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-slate-900/90 border border-slate-700/80 p-6 md:p-8 rounded-3xl w-full max-w-4xl shadow-2xl relative my-8 backdrop-blur-2xl">
            ...
            <form onSubmit={handleSaveStudent} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div><label className="text-xs text-slate-300 mb-2 block">Aadhar No</label><input type="text" name="aadharNo" value={formData.aadharNo} onChange={handleInputChange} className="w-full bg-slate-950 border border-slate-800 px-4 py-3 rounded-xl text-white outline-none focus:border-blue-500" /></div>
              ...
              <button type="submit" disabled={isProcessing} className="btn-glow-blue w-full bg-gradient-to-r from-blue-600 to-indigo-600 py-4 rounded-xl font-bold text-white transition flex justify-center items-center gap-2">{isProcessing ? <Loader2 className="animate-spin"/> : <Save/>} Submit Admission</button>
            </form>
          </div>
        </div>
      )}
      ...
      {!teacherSession && isStaffModalOpen && (
        <div className="fixed inset-0 bg-slate-950/90 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900/90 border border-slate-700/80 p-6 md:p-8 rounded-3xl max-w-md w-full shadow-2xl relative backdrop-blur-2xl">
            ...
            <form onSubmit={handleSaveStaff} className="space-y-4">
              ...
              <button type="submit" disabled={isProcessing} className="btn-glow-purple w-full bg-gradient-to-r from-purple-600 to-fuchsia-600 py-3.5 rounded-xl text-sm font-bold text-white transition flex justify-center items-center gap-2">
      ...
      {isNoticeModalOpen && (
        <div className="fixed inset-0 bg-slate-950/90 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900/90 border border-slate-700/80 p-6 md:p-8 rounded-3xl max-w-lg w-full shadow-2xl relative backdrop-blur-2xl">
            ...
            <form onSubmit={handleSaveNotice} className="space-y-4">
              ...
              <button type="submit" disabled={isProcessing} className="btn-glow-amber w-full bg-gradient-to-r from-amber-600 to-orange-600 py-3.5 rounded-xl text-sm font-bold text-white transition flex justify-center items-center gap-2">
      ...
      {selectedNoticeToPrint && (
        <div className="fixed inset-0 bg-white text-slate-900 z-50 p-6 overflow-y-auto print:p-0">
          <div className="max-w-3xl mx-auto flex justify-between items-center mb-6 no-print">
            <button onClick={() => setSelectedNoticeToPrint(null)} className="bg-slate-200 hover:bg-slate-300 px-4 py-2 rounded-xl font-bold text-xs text-slate-800"><X size={16} className="inline mr-1" /> Close</button>
            <button onClick={() => window.print()} className="btn-glow-blue bg-blue-600 hover:bg-blue-500 text-white px-6 py-2.5 rounded-xl font-bold text-xs flex items-center gap-2"><Printer size={16} /> Print / Save PDF</button>
          </div>
          
          <div className="max-w-3xl mx-auto border-2 border-slate-800 p-12 bg-white relative min-h-[900px] flex flex-col justify-between shadow-xl">
             <div>
               <div className="text-center border-b-2 border-slate-900 pb-6 mb-6">
                  {school.logo_url && <img src={school.logo_url} className="w-20 h-20 mx-auto mb-2 object-contain" alt="Logo" />}
                  <h1 className="text-3xl font-black uppercase text-blue-950 tracking-wide">{school.school_name || 'SCHOOL NAME'}</h1>
                  <p className="text-xs font-medium text-slate-700 mt-1">{school.address}</p>
                  <p className="text-[11px] font-semibold text-slate-600 mt-0.5">Reg No: {school.reg_no || 'N/A'} | Phone: {school.phone} | Email: {school.email || 'N/A'}</p>
               </div>

               <div className="flex justify-between items-center text-xs font-bold text-slate-700 border-b border-dashed border-slate-400 pb-3 mb-6">
                  <p>Ref: <span className="font-mono">NOT-{selectedNoticeToPrint.id || 'OFFICIAL'}</span></p>
                  <span className="bg-slate-900 text-white font-black text-sm uppercase px-6 py-1 rounded-full tracking-widest">OFFICIAL NOTICE</span>
                  <p>Date: <span>{selectedNoticeToPrint.publish_date}</span></p>
               </div>

               <div className="space-y-6 px-2">
                 <h2 className="text-xl font-black text-slate-900 border-l-4 border-blue-900 pl-3 leading-snug">{selectedNoticeToPrint.title}</h2>
                 <p className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Target: {selectedNoticeToPrint.target_class === 'All' ? 'All Students & Guardians' : `Class: ${selectedNoticeToPrint.target_class}`}</p>
                 <div className="text-sm font-medium leading-relaxed text-slate-800 whitespace-pre-line pt-2 text-justify">
                   {selectedNoticeToPrint.content}
                 </div>
               </div>
             </div>

             <div className="flex justify-between items-end pt-20 px-4">
               <div className="text-center">
                 <p className="font-bold text-xs">{selectedNoticeToPrint.publish_date}</p>
                 <div className="w-36 border-t border-slate-900 mt-1"></div>
                 <p className="text-[10px] text-slate-600 uppercase mt-1 font-bold">Issue Date</p>
               </div>
               <div className="text-center">
                 <p className="font-bold text-xs italic">{school.principal_name || 'Head of Institution'}</p>
                 <div className="w-44 border-t border-slate-900 mt-1"></div>
                 <p className="text-[10px] text-slate-600 uppercase mt-1 font-bold">Principal Signature & Seal</p>
               </div>
             </div>
          </div>
        </div>
      ...
      {statusModalStudent && (
        <div className="fixed inset-0 bg-slate-950/90 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900/90 border border-slate-700/80 p-6 rounded-3xl max-w-sm w-full shadow-2xl relative backdrop-blur-2xl">
            ...
            <div className="space-y-4">
              ...
              <button onClick={handleChangeStatus} disabled={isProcessing} className="btn-glow-amber w-full bg-amber-600 hover:bg-amber-500 py-3 rounded-xl text-sm font-bold transition flex justify-center items-center gap-2 mt-4 text-white">
      ...
      {tcPrintData && (
        <div className="fixed inset-0 bg-white text-slate-900 z-50 p-6 overflow-y-auto print:p-0">
          <div className="max-w-2xl mx-auto flex justify-between items-center mb-4 no-print">
            ...
            <button onClick={() => window.print()} className="btn-glow-blue bg-blue-600 text-white px-6 py-2 rounded-lg font-bold text-xs flex items-center gap-2"><Printer size={16} /> Print TC</button>
          </div>
          ...
          <div className="bg-slate-900/90 border border-slate-700/80 p-6 rounded-3xl max-w-sm w-full shadow-2xl relative backdrop-blur-2xl">
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-3 pt-4 border-t border-slate-800">
                <button onClick={() => setEditingTx(null)} className="flex-1 bg-slate-800 hover:bg-slate-700 py-2 rounded-xl text-xs font-bold transition">Cancel</button>
                <button onClick={handleUpdateTx} disabled={isProcessing} className="btn-glow-blue flex-1 bg-blue-600 hover:bg-blue-500 py-2 rounded-xl text-xs font-bold transition flex justify-center items-center">{isProcessing ? <Loader2 className="animate-spin" size={16}/> : 'Save Update'}</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {contactModalStudent && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900/90 border border-slate-800 p-6 rounded-3xl max-w-sm w-full space-y-4 shadow-2xl relative backdrop-blur-2xl">
            ...
            <div className="text-center">
              <img src={contactModalStudent.photo_url || 'https://via.placeholder.com/150'} alt="" className="w-20 h-20 rounded-full mx-auto object-cover border-2 border-blue-500 mb-3 shadow-[0_0_15px_rgba(59,130,246,0.3)]" />
            ...
            <div className="space-y-3 bg-slate-950/70 p-4 rounded-2xl border border-slate-800 text-xs text-slate-300">
            ...
            {contactModalStudent.phone && (
              <a href={`tel:${contactModalStudent.phone}`} className="btn-glow-emerald w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3 rounded-xl flex items-center justify-center gap-2 text-xs transition"><PhoneCall size={16} /> কল করুন</a>
            )}
          </div>
        </div>
      )}
      ...
      {receiptData && (
        <div className="fixed inset-0 bg-white text-slate-900 z-50 p-6 overflow-y-auto print:p-0">
          <div className="max-w-xl mx-auto flex justify-between items-center mb-4 no-print">
            ...
            <button onClick={() => window.print()} className="btn-glow-blue bg-blue-600 text-white px-6 py-2 rounded-lg font-bold text-xs flex items-center gap-2"><Printer size={16} /> রসিদ প্রিন্ট / PDF</button>
          </div>
      ...
      {printIdCard && selectedIdStudent && (
        <div className="fixed inset-0 bg-slate-950/95 z-50 flex items-center justify-center p-4 pvc-modal-overlay">
          <div className="absolute top-6 right-6 flex gap-4 no-print">
            <button onClick={() => setPrintIdCard(false)} className="bg-slate-800 px-5 py-2.5 rounded-xl font-bold text-white hover:bg-slate-700 transition"><X size={16} className="inline mr-2" /> Cancel</button>
            <button onClick={() => window.print()} className="btn-glow-blue bg-blue-600 px-5 py-2.5 rounded-xl font-bold text-white shadow-lg shadow-blue-500/30 hover:bg-blue-500 transition"><Printer size={16} className="inline mr-2" /> Print PVC (Single Page)</button>
          </div>
      ...
      {/* SIDEBAR NAVIGATION */}
      <aside className="w-full md:w-72 bg-slate-900/80 backdrop-blur-2xl border-r border-slate-800/80 p-6 flex flex-col justify-between relative z-20">
        <div>
          <div className="flex items-center gap-3 mb-8">
            {school.logo_url ? <img src={school.logo_url} alt="Logo" className="w-12 h-12 rounded-xl object-cover bg-white p-1 shadow-[0_0_15px_rgba(59,130,246,0.3)]" /> : <div className="bg-gradient-to-tr from-blue-600 to-indigo-500 p-3 rounded-2xl shadow-[0_0_20px_rgba(59,130,246,0.4)]"><School className="text-white" size={26} /></div>}
            <div>
              <h1 className="text-xl font-black bg-gradient-to-r from-blue-400 via-indigo-300 to-purple-300 bg-clip-text text-transparent">{school.school_name || 'EduAdmin'}</h1>
            ...
            {navItems.map((item) => {
              ...
              return (
                <button 
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-bold transition-all duration-300 ${ activeTab === item.id ? 'btn-glow-blue bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 text-white scale-[1.02]' : 'text-slate-400 hover:bg-slate-800/60 hover:text-white'}`}
                >
      ...
      <main className="flex-1 p-6 md:p-10 overflow-y-auto w-full relative z-10">
        ...
        {activeTab === 'dashboard' && (
          <div className="space-y-8 animate-fade-in">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-slate-800 pb-4 gap-4">
               <div>
                 <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full border border-indigo-500/30 bg-indigo-500/10 text-indigo-300 text-[11px] font-bold uppercase tracking-wider mb-2">
                   <Sparkles size={12}/> Live Overview
                 </div>
                 <h2 className="text-3xl font-black text-white">Dashboard & Analytics</h2>
                 <p className="text-sm text-slate-400 mt-0.5">Welcome back to your school portal.</p>
               </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="bg-slate-900/60 border border-slate-800/90 p-6 rounded-3xl flex items-center gap-4 hover:border-blue-500/40 hover:shadow-[0_0_30px_-5px_rgba(59,130,246,0.2)] transition backdrop-blur-xl">
                <div className="p-4 bg-blue-500/10 text-blue-400 rounded-2xl"><Users size={28} /></div>
              ...
              <div className="bg-slate-900/60 border border-slate-800/90 p-6 rounded-3xl flex items-center gap-4 hover:border-purple-500/40 hover:shadow-[0_0_30px_-5px_rgba(168,85,247,0.2)] transition backdrop-blur-xl">
                <div className="p-4 bg-purple-500/10 text-purple-400 rounded-2xl"><Briefcase size={28} /></div>
                <div><p className="text-xs text-slate-400 font-bold uppercase tracking-wider">{teacherSession ? 'My Role' : 'Total Staff'}</p><h3 className="text-2xl font-black text-purple-400 mt-1">{teacherSession ? teacherSession.role : staffList.length}</h3></div>
              </div>
              <div className="bg-slate-900/60 border border-slate-800/90 p-6 rounded-3xl flex items-center gap-4 hover:border-amber-500/40 hover:shadow-[0_0_30px_-5px_rgba(245,158,11,0.2)] transition backdrop-blur-xl">
                <div className="p-4 bg-amber-500/10 text-amber-400 rounded-2xl"><Megaphone size={28} /></div>
                <div><p className="text-xs text-slate-400 font-bold uppercase tracking-wider">Published Notices</p><h3 className="text-3xl font-black text-amber-400 mt-1">{notices.length}</h3></div>
              </div>
            </div>

            {/* NOTICES SECTION */}
            <div className="bg-slate-900/60 border border-slate-800/90 p-6 rounded-3xl shadow-xl space-y-4 backdrop-blur-xl">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {notices.length > 0 ? notices.map((n) => (
                  <div key={n.id} className="bg-slate-950/80 p-5 rounded-2xl border border-slate-800/80 flex flex-col justify-between space-y-3 hover:border-slate-700 transition">
                    <div>
                      <div className="flex justify-between items-start gap-2">
                        <h4 className="font-bold text-white text-base leading-snug">{n.title}</h4>
                        <span className="bg-slate-800 text-slate-300 px-2 py-0.5 rounded text-[10px] font-bold whitespace-nowrap">{n.publish_date}</span>
                      </div>
                      <p className="text-xs text-slate-400 mt-2 leading-relaxed whitespace-pre-line line-clamp-3">{n.content}</p>
                    </div>
                    <div className="flex items-center justify-between pt-2 border-t border-slate-900">
                      <span className="text-[10px] text-slate-500 font-semibold">For: {n.target_class || 'All'}</span>
                      <div className="flex gap-2">
                        <button onClick={() => setSelectedNoticeToPrint(n)} className="btn-glow-blue bg-blue-600/20 hover:bg-blue-600/30 text-blue-400 border border-blue-500/30 px-3 py-1.5 rounded-xl text-xs font-bold flex items-center gap-1 transition">
            ...
            {/* CHART */}
            <div className="bg-slate-900/60 border border-slate-800/90 p-6 rounded-3xl shadow-xl h-[400px] backdrop-blur-xl">
        ...
        {activeTab === 'students' && (
          <div className="space-y-6 animate-fade-in w-full">
            ...
            <div className="flex bg-slate-950/80 border border-slate-800 p-1.5 rounded-2xl w-fit">
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-4">
              <div className="relative w-full md:flex-1">
                ...
                <input type="text" placeholder="Search by name, roll or Unique ID..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-slate-900 border border-slate-800 pl-12 pr-4 py-3 rounded-2xl text-sm text-white focus:outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
              ...
              <div className="flex gap-3 w-full md:w-auto overflow-x-auto scrollbar-hide pb-2 md:pb-0">
                ...
                <select onChange={(e) => setSelectedClassFilter(e.target.value)} value={selectedClassFilter} className="bg-slate-900 text-white border border-slate-800 px-4 py-2.5 rounded-2xl text-xs font-bold outline-none">
            ...
            <div className="bg-slate-900/60 border border-slate-800 rounded-3xl overflow-x-auto shadow-xl backdrop-blur-xl">
        ...
        {activeTab === 'attendance' && (
          <div className="space-y-6 animate-fade-in w-full">
            ...
            <div className="bg-slate-900/60 p-6 rounded-3xl border border-slate-800 flex flex-wrap gap-4 items-end backdrop-blur-xl">
               <div>
                  <label className="text-xs text-slate-400 block mb-1 font-bold">Select Class</label>
                  <select value={attendanceClass} onChange={e => {setAttendanceClass(e.target.value); loadAttendance(e.target.value, attendanceDate);}} className="bg-slate-950 p-3 rounded-2xl border border-slate-800 text-white outline-none font-bold">
               ...
               <div>
                  ...
                  <input type="date" value={attendanceDate} onChange={e => {setAttendanceDate(e.target.value); loadAttendance(attendanceClass, e.target.value);}} className="bg-slate-950 p-3 rounded-2xl border border-slate-800 text-white outline-none font-bold"/>
               </div>
            </div>
                    ...
                    {students.filter(s => s.student_class === attendanceClass && s.status === 'Active').map(st => (
                      <tr key={st.id} className="hover:bg-slate-800/20 transition">
                        ...
                        <td className="py-4 px-2 text-right flex justify-end gap-2">
                          <button onClick={() => handleSaveAttendance(st.id, 'Present')} className={`px-5 py-2 rounded-2xl text-xs font-bold transition ${attendanceRecords[st.id] === 'Present' ? 'btn-glow-emerald bg-emerald-600 text-white' : 'bg-slate-800 text-slate-400 hover:bg-slate-700'}`}>Present</button>
                          <button onClick={() => handleSaveAttendance(st.id, 'Absent')} className={`px-5 py-2 rounded-2xl text-xs font-bold transition ${attendanceRecords[st.id] === 'Absent' ? 'btn-glow-rose bg-rose-600 text-white' : 'bg-slate-800 text-slate-400 hover:bg-slate-700'}`}>Absent</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
        ...
        {!teacherSession && activeTab === 'class_mgmt' && (
          <div className="space-y-6 animate-fade-in w-full">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="bg-slate-900/60 border border-slate-800 p-6 rounded-3xl space-y-4 backdrop-blur-xl">
        ...
        {!teacherSession && activeTab === 'erp' && (
          <div className="space-y-6 animate-fade-in w-full">
            <h2 className="text-2xl font-bold border-b border-slate-800 pb-4">Financial Operations & Revenue</h2>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-slate-900/60 border border-slate-800 p-6 rounded-3xl flex items-center justify-between shadow-xl backdrop-blur-xl">
                <div className="flex items-center gap-4">
                  <div className="p-4 bg-emerald-500/10 text-emerald-400 rounded-2xl"><DollarSign size={32} /></div>
                  <div>
              ...
              <div className="bg-slate-900/60 border border-slate-800 p-6 rounded-3xl flex items-center justify-between shadow-xl backdrop-blur-xl">
                <div className="flex items-center gap-4">
                  <div className="p-4 bg-rose-500/10 text-rose-400 rounded-2xl"><AlertCircle size={32} /></div>
                  <div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                {erpStudent && erpClassConfig && (
                  <div className="lg:col-span-8 space-y-6">
                    <div className="bg-slate-900/60 border border-slate-800 p-6 rounded-3xl shadow-xl overflow-x-auto backdrop-blur-xl">
                    ...
                    <div className="bg-slate-900/60 border border-slate-800 p-6 rounded-3xl shadow-xl space-y-6 backdrop-blur-xl">
              ...
              {selectedIdStudent && (
                <div className="pt-8 border-t border-slate-800 flex flex-col md:flex-row gap-10 items-center md:items-start">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-400 mb-6 max-w-sm leading-relaxed">The card is calibrated for standard ISO CR80 (54x85.6mm) PVC printing. It prints on a single page with vibrant clarity.</p>
                    <button onClick={() => setPrintIdCard(true)} className="btn-glow-blue bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 px-6 py-4 rounded-2xl font-black text-white transition flex items-center gap-3">
        ...
        {!teacherSession && activeTab === 'profile' && (
          <div className="space-y-6 animate-fade-in w-full pb-10">
            ...
            {editSchool ? (
              <div className="bg-slate-900/80 border border-slate-800 p-6 md:p-8 rounded-3xl shadow-2xl backdrop-blur-xl">
              ...
              <div className="bg-slate-900/60 border border-slate-800 rounded-3xl shadow-2xl overflow-hidden relative backdrop-blur-xl">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 p-6 md:p-10">
                  ...
                  <div className="bg-slate-950/50 border border-slate-800/80 p-6 rounded-2xl hover:border-slate-700 transition flex flex-col">
                    ...
                    <div className="mt-auto pt-6">
                      <div className="p-4 bg-gradient-to-br from-blue-900/30 to-indigo-900/30 border border-blue-500/20 rounded-2xl relative overflow-hidden">
