...
import { 
  ...
  Activity, Bell, CalendarCheck, Briefcase, UploadCloud, BarChart3, Download, Lock, Megaphone
} from 'lucide-react';
...
export default function Dashboard() {
  ...
  // ==============================================================
  // STUDENT PORTAL VIEW (WITH NOTICES & LETTERHEAD PDF DOWNLOAD)
  // ==============================================================
  if (studentSession) {
          ...
          <div className="bg-slate-900/60 border border-slate-800 p-6 rounded-3xl shadow-xl space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {studentNotices.length > 0 ? studentNotices.map((n) => (
                <div key={n.id} className="bg-slate-950 p-5 rounded-2xl border border-slate-800 flex flex-col justify-between space-y-3 hover:border-amber-500/40 transition">
                  <div>
                  ...
                  <button onClick={() => setSelectedNoticeToPrint(n)} className="bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-xl text-xs font-bold flex items-center justify-center gap-2 transition w-fit shadow-md">
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-slate-950 text-slate-100 font-sans">
      ...
      {/* STAFF MANAGEMENT MODAL (WITH LOGIN CREDENTIALS) */}
      {isStaffModalOpen && (
        <div className="fixed inset-0 bg-slate-950/90 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-700 p-6 md:p-8 rounded-2xl max-w-md w-full shadow-2xl relative">
            ...
            <h3 className="text-lg font-bold text-purple-400 mb-4 flex items-center gap-2"><Briefcase size={20}/> {staffData.id ? 'Edit Staff Profile' : 'Add New Teacher / Staff'}</h3>
            <p className="text-xs text-slate-400 mb-4 leading-relaxed">শিক্ষক বা স্টাফরা তাদের মোবাইল নম্বর এবং অ্যাডমিনের দেওয়া পাসওয়ার্ড দিয়ে লগইন পেজ থেকে লগইন করতে পারবেন।</p>
            <form onSubmit={handleSaveStaff} className="space-y-4">
              ...
              <button type="submit" disabled={isProcessing} className="w-full bg-purple-600 hover:bg-purple-500 py-3.5 rounded-xl text-sm font-bold text-white transition flex justify-center items-center gap-2 shadow-lg shadow-purple-600/30">
      ...
      {isNoticeModalOpen && (
        <div className="fixed inset-0 bg-slate-950/90 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-700 p-6 md:p-8 rounded-2xl max-w-lg w-full shadow-2xl relative">
            <button onClick={() => setIsNoticeModalOpen(false)} className="absolute top-4 right-4 text-slate-400 hover:text-white"><X size={18} /></button>
            ...
            <p className="text-xs text-slate-400 mb-4">এই নোটিশটি ছাত্রছাত্রীদের ড্যাশবোর্ডে প্রকাশিত হবে এবং অফিশিয়াল লেটারহেড সহ PDF প্রিন্ট করা যাবে।</p>
            <form onSubmit={handleSaveNotice} className="space-y-4">
              <div>
                <label className="text-xs text-slate-300 block mb-1 font-semibold">Notice Title (শিরোনাম) *</label>
                <input type="text" value={noticeFormData.title} onChange={e => setNoticeFormData({...noticeFormData, title: e.target.value})} className="w-full bg-slate-950 border border-slate-700 p-3 rounded-xl text-white text-sm outline-none focus:border-amber-500 font-bold" placeholder="e.g. Eid Vacation Notice / Exam Date Announcement" required/>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
              ...
              <div>
                ...
                <textarea rows={5} value={noticeFormData.content} onChange={e => setNoticeFormData({...noticeFormData, content: e.target.value})} className="w-full bg-slate-950 border border-slate-700 p-3 rounded-xl text-white text-sm outline-none focus:border-amber-500 resize-none leading-relaxed" placeholder="Write full notice details here..." required/>
              </div>
              <button type="submit" disabled={isProcessing} className="w-full bg-amber-600 hover:bg-amber-500 py-3.5 rounded-xl text-sm font-bold text-white transition flex justify-center items-center gap-2 shadow-lg shadow-amber-600/30">
      ...
      {/* OFFICIAL NOTICE PRINT MODAL (WITH SCHOOL LETTERHEAD) */}
      {selectedNoticeToPrint && (
        <div className="fixed inset-0 bg-white text-slate-900 z-50 p-6 overflow-y-auto print:p-0">
          <div className="max-w-3xl mx-auto flex justify-between items-center mb-6 no-print">
            ...
            <button onClick={() => window.print()} className="bg-blue-600 hover:bg-blue-500 text-white px-6 py-2.5 rounded-xl font-bold text-xs flex items-center gap-2 shadow-lg shadow-blue-500/20"><Printer size={16} /> Print / Save PDF</button>
          </div>
      ...
      {/* PVC ID CARD PRINT MODAL (VIBRANT & SINGLE PAGE PDF GUARANTEED) */}
      {printIdCard && selectedIdStudent && (
        <div className="fixed inset-0 bg-slate-950/95 z-50 flex items-center justify-center p-4 pvc-modal-overlay">
          <div className="absolute top-6 right-6 flex gap-4 no-print">
            ...
            <button onClick={() => window.print()} className="bg-blue-600 px-5 py-2.5 rounded-xl font-bold text-white shadow-lg shadow-blue-500/30 hover:bg-blue-500 transition"><Printer size={16} className="inline mr-2" /> Print PVC (Single Page)</button>
          </div>
          ...
          <div className="pvc-card w-[204px] h-[323px] bg-gradient-to-br from-indigo-950 via-slate-900 to-blue-950 overflow-hidden relative flex flex-col text-white rounded-2xl border border-indigo-500/30 shadow-2xl shrink-0">
             ...
             {/* Student Photo with Vibrant Multi-Color Gradient Ring */}
             ...
             {/* Bottom Multi-color Rainbow Bar */}
      ...
      <main className="flex-1 p-6 md:p-10 overflow-y-auto w-full">
        ...
        {/* DASHBOARD TAB (OVERVIEW WITH NOTICES PUBLISHED & STATS) */}
        {activeTab === 'dashboard' && (
          <div className="space-y-8 animate-fade-in">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-slate-800 pb-4 gap-4">
               <div>
                 <h2 className="text-3xl font-bold text-white">Dashboard & Analytics</h2>
                 <p className="text-sm text-slate-400 mt-1">Welcome back to your school portal.</p>
               </div>
               <div className="flex gap-3 flex-wrap">
                 <button onClick={() => setIsNoticeModalOpen(true)} className="bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-500 hover:to-orange-500 px-5 py-3 rounded-xl font-bold text-white flex items-center gap-2 shadow-lg shadow-amber-600/20 transition">
                   <Megaphone size={18}/> Publish Notice
                 </button>
                 <button onClick={() => setIsAdmissionModalOpen(true)} className="bg-gradient-to-r from-blue-600 to-indigo-600 px-6 py-3 rounded-xl font-bold text-white flex items-center gap-2 shadow-lg shadow-blue-500/20 transition">
                   <Plus size={20}/> New Admission
                 </button>
               </div>
            </div>
            
            {/* Overview Stats (Revenue & Due moved to ERP) */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="bg-slate-900/60 border border-slate-800 p-6 rounded-2xl flex items-center gap-4">
                ...
                <div><p className="text-xs text-slate-400 font-semibold uppercase tracking-wider">Active Students</p><h3 className="text-3xl font-black text-white mt-1">{activeStudentsList.length}</h3></div>
              </div>
              <div className="bg-slate-900/60 border border-slate-800 p-6 rounded-2xl flex items-center gap-4">
                <div className="p-4 bg-purple-500/10 text-purple-400 rounded-xl"><Briefcase size={28} /></div>
                <div><p className="text-xs text-slate-400 font-semibold uppercase tracking-wider">Total Staff & Teachers</p><h3 className="text-3xl font-black text-purple-400 mt-1">{staffList.length}</h3></div>
              </div>
              <div className="bg-slate-900/60 border border-slate-800 p-6 rounded-2xl flex items-center gap-4">
                <div className="p-4 bg-amber-500/10 text-amber-400 rounded-xl"><Megaphone size={28} /></div>
                <div><p className="text-xs text-slate-400 font-semibold uppercase tracking-wider">Published Notices</p><h3 className="text-3xl font-black text-amber-400 mt-1">{notices.length}</h3></div>
              </div>
            </div>
            ...
            <div className="bg-slate-900/60 border border-slate-800 p-6 rounded-2xl shadow-xl space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {notices.length > 0 ? notices.map((n) => (
                  <div key={n.id} className="bg-slate-950 p-5 rounded-2xl border border-slate-800 flex flex-col justify-between space-y-3 hover:border-slate-700 transition">
                    <div>
                      <div className="flex justify-between items-start gap-2">
                        <h4 className="font-bold text-white text-base leading-snug">{n.title}</h4>
                    ...
                    <div className="flex items-center justify-between pt-2 border-t border-slate-900">
                      ...
                      <div className="flex gap-2">
                        <button onClick={() => setSelectedNoticeToPrint(n)} className="bg-blue-600/20 hover:bg-blue-600/30 text-blue-400 border border-blue-500/30 px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1 transition">
                          <Printer size={13}/> Letterhead PDF
                        </button>
                        <button onClick={() => handleDeleteNotice(n.id)} className="bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 p-1.5 rounded-lg transition">
                          <Trash2 size={14}/>
                        </button>
                      </div>
                    </div>
                  ...
                  <div className="col-span-2 text-center p-8 text-slate-500 border border-dashed border-slate-800 rounded-2xl">
                    <Megaphone size={32} className="mx-auto mb-2 opacity-30"/>
                    <p className="text-xs font-medium">কোনো নোটিশ এখনো পাবলিশ করা হয়নি। "Publish Notice" বাটনে ক্লিক করে প্রথম নোটিশ প্রকাশ করুন।</p>
                  </div>
                )}
              </div>
            </div>
        ...
        {/* STAFF & TEACHERS TAB (WITH LOGIN CREDENTIALS) */}
        {activeTab === 'staff' && (
          <div className="space-y-6 animate-fade-in w-full">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-slate-800 pb-4 gap-4">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-1">শিক্ষক ও স্টাফদের অ্যাকাউন্ট তৈরি করুন। শিক্ষকরা তাদের মোবাইল ও পাসওয়ার্ড দিয়ে লগইন পেজ থেকে লগইন করতে পারবেন।</p>
              </div>
              <button onClick={() => { setStaffData({ id: null, name: '', role: 'Teacher', phone: '', password: '', salary: 0, status: 'Active' }); setIsStaffModalOpen(true); }} className="bg-purple-600 hover:bg-purple-500 px-5 py-2.5 rounded-xl font-bold text-white flex items-center gap-2 shadow-lg shadow-purple-500/20 transition">
                  ...
                  {staffList.map((stf) => (
                    <tr key={stf.id} className="hover:bg-slate-800/30 transition">
                      ...
                      <td className="p-4 flex gap-2 justify-center">
                        ...
                        }} className="text-red-400 p-2 bg-red-500/10 hover:bg-red-500/20 rounded-lg transition"><Trash2 size={16} /></button>
                      </td>
                    </tr>
                  ))}
                  {staffList.length === 0 && (
                    <tr><td colSpan="5" className="text-center p-12 text-slate-500"><Briefcase size={40} className="mx-auto mb-3 opacity-20"/><p className="font-semibold">No staff records found. Add your first staff member!</p></td></tr>
                  )}
                </tbody>
              </table>
            </div>
          ...
          <div className="space-y-6 animate-fade-in w-full">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-slate-900/60 border border-slate-800 p-6 rounded-2xl flex items-center justify-between shadow-xl">
                <div className="flex items-center gap-4">
                  <div className="p-4 bg-emerald-500/10 text-emerald-400 rounded-xl"><DollarSign size={32} /></div>
                  <div>
              ...
              <div className="bg-slate-900/60 border border-slate-800 p-6 rounded-2xl flex items-center justify-between shadow-xl">
                <div className="flex items-center gap-4">
                  <div className="p-4 bg-rose-500/10 text-rose-400 rounded-xl"><AlertCircle size={32} /></div>
                  <div>
                ...
                <button onClick={() => setShowOnlyPendingList(!showOnlyPendingList)} className="bg-rose-600/20 hover:bg-rose-600/30 text-rose-400 border border-rose-500/30 px-4 py-2.5 rounded-xl text-xs font-bold transition">
              ...
              {selectedIdStudent && (
                <div className="pt-8 border-t border-slate-800 flex flex-col md:flex-row gap-10 items-center md:items-start">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-400 mb-6 max-w-sm leading-relaxed">The card is calibrated for standard ISO CR80 (54x85.6mm) PVC printing. It prints on a single page with high color fidelity.</p>
