...
export default function Dashboard() {
  ...
  const handleExportCSV = () => {
    ...
    const csvRows = dataToExport.map(st => [
      st.unique_id || 'N/A',
      st.roll_no || '',
      `"${st.name || ''}"`,
      st.student_class || '',
    ...
    showToast('ডাটা সফলভাবে এক্সেল (CSV) ফাইলে এক্সপোর্ট হয়েছে!');
  };
      ...
      <main className="flex-1 p-6 md:p-10 overflow-y-auto w-full">
        ...
        {activeTab === 'students' && (
          <div className="space-y-6 animate-fade-in w-full">
            ...
            {/* NEW EXCEL EXPORT ROW INCLUDED HERE */}
            <div className="flex flex-col md:flex-row justify-between items-center gap-4">
              ...
              <div className="flex gap-3 w-full md:w-auto">
                {/* NEW EXPORT BUTTON */}
                <button onClick={handleExportCSV} className="w-full md:w-auto bg-slate-800 hover:bg-slate-700 text-emerald-400 border border-emerald-500/30 px-5 py-3 rounded-xl text-xs font-bold flex justify-center items-center gap-2 transition whitespace-nowrap">
                  <FileSpreadsheet size={16} /> Export to Excel
                </button>

                {selectedClassFilter !== 'All' && studentStatusTab === 'Active' && (
                  <button onClick={() => handleUpgradeClass(selectedClassFilter)} className="w-full md:w-auto bg-emerald-600 hover:bg-emerald-500 px-5 py-3 rounded-xl text-xs font-bold flex justify-center items-center gap-2 shadow-lg shadow-emerald-500/20 whitespace-nowrap transition">
                    <ArrowUpCircle size={16} /> Upgrade {selectedClassFilter}
                  </button>
                )}
              </div>
            </div>
