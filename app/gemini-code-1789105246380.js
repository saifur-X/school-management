...
import { 
  ...
  Activity, Bell, CalendarCheck, Briefcase, UploadCloud, BarChart3, Award
} from 'lucide-react';
...
export default function Dashboard() {
  ...
  // MARK ENTRY METHODS (SAVE & LOAD)
  const loadClassMarks = async (cls, exam) => {
    if (!cls || !exam) return;
    ...
    if (data && data.length > 0) {
      data.forEach(item => {
        marksMap[item.student_id] = item.marks_data || {};
      });
    }
  ...
  const handleMarkInputChange = (studentId, key, val) => {
  ...
  const handleSaveStudentMark = async (st) => {
    if (!markEntryClass || !markEntryExam) return showToast('ক্লাস ও পরীক্ষা সিলেক্ট করুন', 'error');
    setIsProcessing(true);
    const marksData = currentMarks[st.id] || {};
    const subjects = allClassConfigs[markEntryClass]?.subjects || [];
    let total = 0;
    subjects.forEach(sub => {
      total += Number(marksData[`${sub.name}_theory`] || 0) + Number(marksData[`${sub.name}_oral`] || 0);
    });
    ...
    if (existing?.id) {
      const res = await supabase
        ...
        .update({
          ...
          total_marks: total,
          student_class: markEntryClass,
          academic_year: allClassConfigs[markEntryClass]?.academic_year || '2026'
        })
        .eq('id', existing.id);
      err = res.error;
    } else {
    ...
    if (!err) {
      showToast(`${st.name} এর মার্ক সেভ হয়েছে!`);
    } else {
      showToast('মার্ক সেভ ব্যর্থ: ' + err.message, 'error');
    }
    setIsProcessing(false);
  };

  const handleSaveAllMarks = async () => {
    const classStudents = students.filter(s => s.student_class === markEntryClass && (s.status === 'Active' || !s.status));
    if (classStudents.length === 0) return showToast('কোনো স্টুডেন্ট নেই!', 'error');
    setIsProcessing(true);
    try {
      for (const st of classStudents) {
        await handleSaveStudentMark(st);
      }
      showToast('সম্পূর্ণ ক্লাসের মার্ক সফলভাবে ডাটাবেজে সেভ হয়েছে!');
    } catch (e) {
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-slate-950 text-slate-100 font-sans">
      ...
      {/* PVC ID CARD PRINT MODAL (VIBRANT & SINGLE PAGE PDF READY) */}
      {printIdCard && selectedIdStudent && (
        <div className="fixed inset-0 bg-slate-950/95 z-50 flex items-center justify-center p-4 pvc-modal-overlay">
          ...
          <div className="pvc-card w-[204px] h-[323px] bg-gradient-to-br from-indigo-950 via-slate-900 to-blue-950 overflow-hidden relative flex flex-col text-white rounded-2xl border border-indigo-500/30 shadow-2xl shrink-0">
             ...
             <div className="flex justify-center mt-2.5 z-10">
             ...
             <div className="bg-slate-900/80 backdrop-blur-md mx-2.5 rounded-xl p-2 text-[7.5px] space-y-1 mt-2 z-10 border border-indigo-500/20 shadow-inner">
        ...
        <div>
          ...
          <nav className="space-y-2 max-h-[60vh] overflow-y-auto scrollbar-hide pr-2">
            {[
              ...
              { id: 'mark_entry', label: 'Mark Entry & Report', icon: FileSpreadsheet },
      ...
      <main className="flex-1 p-6 md:p-10 overflow-y-auto w-full">
        ...
        {/* MARK ENTRY & TABULATION TAB (FIXED & PERSISTENT) */}
        {activeTab === 'mark_entry' && (
          <div className="space-y-6 animate-fade-in w-full">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-slate-800 pb-4 gap-4">
              <div>
                <h2 className="text-2xl font-bold text-white flex items-center gap-2"><Award className="text-emerald-400" size={24}/> Mark Entry & Evaluation</h2>
                <p className="text-xs text-slate-400 mt-1">সব বিষয়ের নম্বর ইনপুট দিন, ডাটা স্বয়ংক্রিয়ভাবে ডাটাবেজে পার্মানেন্ট সেভ থাকবে।</p>
              </div>
              {markEntryClass && (
                <button onClick={handleSaveAllMarks} disabled={isProcessing} className="bg-emerald-600 hover:bg-emerald-500 text-white px-6 py-2.5 rounded-xl font-bold text-xs flex items-center gap-2 shadow-lg shadow-emerald-500/20 transition">
                  {isProcessing ? <Loader2 className="animate-spin" size={16}/> : <Save size={16}/>} Save All Class Marks
                </button>
              )}
            </div>

            {/* Selection Bars */}
            <div className="bg-slate-900/60 p-6 rounded-2xl border border-slate-800 grid grid-cols-1 md:grid-cols-2 gap-6 shadow-xl">
              <div>
                <label className="text-xs text-slate-400 block mb-2 font-bold uppercase tracking-wider">Step 1: Select Class</label>
                <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                  {classList.map(cls => (
                    <button key={cls} onClick={() => { setMarkEntryClass(cls); loadClassMarks(cls, markEntryExam); }} className={`px-4 py-2.5 rounded-xl text-xs font-bold whitespace-nowrap transition ${markEntryClass === cls ? 'bg-emerald-600 text-white shadow-lg' : 'bg-slate-950 text-slate-400 border border-slate-800 hover:bg-slate-800'}`}>
                      {cls}
                    </button>
                  ))}
                </div>
              ...
              <div>
                <label className="text-xs text-slate-400 block mb-2 font-bold uppercase tracking-wider">Step 2: Select Examination</label>
                <select value={markEntryExam} onChange={e => { setMarkEntryExam(e.target.value); loadClassMarks(markEntryClass, e.target.value); }} className="w-full bg-slate-950 border border-slate-800 p-3 rounded-xl text-white text-sm outline-none focus:border-emerald-500 font-bold">
            ...
            {markEntryClass && (
              <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-4">
                <div className="flex justify-between items-center">
                  <h3 className="text-sm font-bold text-slate-300">Class: <span className="text-emerald-400">{markEntryClass}</span> | Exam: <span className="text-amber-400">{markEntryExam}</span></h3>
                  {isLoadingMarks && <span className="text-xs text-blue-400 flex items-center gap-1"><Loader2 className="animate-spin" size={14}/> Loading previous marks...</span>}
                </div>
                  ...
                  <table className="w-full text-left text-xs min-w-[750px]">
                    <thead className="bg-slate-800/80 text-slate-400 uppercase tracking-wider">
                      <tr>
                        ...
                        {(allClassConfigs[markEntryClass]?.subjects || []).map((sub, i) => (
                          <th key={i} className="p-3 text-center border-l border-slate-700">
                            {sub.name}
                            <span className="block text-[9px] text-slate-500 lowercase mt-0.5">th:{sub.theory} | or:{sub.oral}</span>
                          </th>
                        ))}
                        <th className="p-3 text-center border-l border-slate-700">Total</th>
                        <th className="p-3 text-center border-l border-slate-700">Action</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-800 bg-slate-950/40">
                      {students.filter(s => s.student_class === markEntryClass && (s.status === 'Active' || !s.status)).map(st => {
                        const stMarks = currentMarks[st.id] || {};
                        const subjects = allClassConfigs[markEntryClass]?.subjects || [];
                        let rowTotal = 0;
                        subjects.forEach(sub => {
                          rowTotal += Number(stMarks[`${sub.name}_theory`] || 0) + Number(stMarks[`${sub.name}_oral`] || 0);
                        });

                        return (
                          <tr key={st.id} className="hover:bg-slate-800/30 transition">
                            <td className="p-3 font-semibold text-white whitespace-nowrap">
                              <span className="text-slate-500 font-bold mr-2">#{st.roll_no}</span>
                              {st.name}
                            </td>

                            {subjects.map((sub, sIdx) => (
                              <td key={sIdx} className="p-2 text-center border-l border-slate-800/60">
                                <div className="flex gap-1 justify-center items-center">
                                  <input 
                                    type="number" 
                                    placeholder="Th" 
                                    value={stMarks[`${sub.name}_theory`] ?? ''} 
                                    onChange={e => handleMarkInputChange(st.id, `${sub.name}_theory`, e.target.value)} 
                                    className="w-12 bg-slate-900 border border-slate-700 p-1.5 rounded text-center text-white outline-none focus:border-emerald-500 font-bold text-xs" 
                                  />
                                  <input 
                                    type="number" 
                                    placeholder="Or" 
                                    value={stMarks[`${sub.name}_oral`] ?? ''} 
                                    onChange={e => handleMarkInputChange(st.id, `${sub.name}_oral`, e.target.value)} 
                                    className="w-12 bg-slate-900 border border-slate-700 p-1.5 rounded text-center text-amber-300 outline-none focus:border-amber-500 font-bold text-xs" 
                                  />
                                </div>
                              </td>
                            ))}

                            <td className="p-3 text-center border-l border-slate-800/60 font-black text-emerald-400 text-sm">
                              {rowTotal}
                            </td>

                            <td className="p-3 text-center border-l border-slate-800/60">
                              <button onClick={() => handleSaveStudentMark(st)} disabled={isProcessing} className="bg-blue-600/20 hover:bg-blue-600/40 text-blue-400 border border-blue-500/30 px-3 py-1.5 rounded-lg text-[11px] font-bold transition inline-flex items-center gap-1">
                                <Save size={12}/> Save
                              </button>
                            </td>
                          </tr>
                      ...
                      {students.filter(s => s.student_class === markEntryClass && (s.status === 'Active' || !s.status)).length === 0 && (
                        <tr><td colSpan="10" className="text-center py-8 text-slate-500 font-semibold">এই ক্লাসে কোনো সক্রিয় স্টুডেন্ট পাওয়া যায়নি।</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
              ...
              {selectedIdStudent && (
                <div className="pt-8 border-t border-slate-800 flex flex-col md:flex-row gap-10 items-center md:items-start">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-400 mb-6 max-w-sm leading-relaxed">The card is perfectly calibrated for ISO CR80 standard PVC printing. Fixed to print strictly in 1 page without extra paper wastage.</p>
