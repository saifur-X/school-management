...
export default function Dashboard() {
  ...
  const [portalUrl, setPortalUrl] = useState('https://eduadmin.vercel.app'); // Default URL
  ...
  useEffect(() => {
    // Set actual portal URL for QR Code
  ...
  // Admin ERP Logistics - FIXED NaN BUGS
  const handleSelectErpStudent = async (st) => {
    ...
    const aMonthly = st.agreed_monthly_fee != null ? Number(st.agreed_monthly_fee) : Number(cConfig.tuition_fee || 0);
    const aAdmission = st.agreed_admission_fee != null ? Number(st.agreed_admission_fee) : Number(cConfig.admission_fee || 0);
  ...
  const handleCreateInvoice = async () => {
    ...
    for (let i = 0; i < erpSelectedFeeTypes.length; i++) {
      ...
      const payable = feeType === 'Tuition Fee' ? (agreedFees.monthly * Math.max(1, (new Date().getMonth() + 1 - (erpClassConfig?.start_month || 1)) + 1)) : feeType === 'Admission Fee' ? agreedFees.admission : Number(erpClassConfig?.custom_fee || 0);
  ...
  const calculateTotalDues = (selectedTypes, configData, aMonthly, aAdmission, studentTx = []) => {
    ...
    selectedTypes.forEach(feeType => {
      const payable = feeType === 'Tuition Fee' ? (aMonthly * Math.max(1, (new Date().getMonth() + 1 - (configData?.start_month || 1)) + 1)) : feeType === 'Admission Fee' ? aAdmission : Number(configData?.custom_fee || 0);
  ...
  const getMonthlyStatus = (monthIndex, studentObj = null, txData = null, cConfig = null) => {
    ...
    const feePerMonth = st?.agreed_monthly_fee != null ? Number(st.agreed_monthly_fee) : Number(config?.tuition_fee || 0);
  ...
  if (studentSession) {
    ...
    const aMonthly = studentSession.agreed_monthly_fee != null ? Number(studentSession.agreed_monthly_fee) : Number(config.tuition_fee || 0);
    const aAdmission = studentSession.agreed_admission_fee != null ? Number(studentSession.agreed_admission_fee) : Number(config.admission_fee || 0);
  ...
  const generatePendingDuesArray = () => {
    ...
    activeStudentsList.forEach(st => {
      ...
      const tuitionDue = Math.max(0, ((st.agreed_monthly_fee != null ? Number(st.agreed_monthly_fee) : Number(config.tuition_fee || 0)) * activeMonths) - studentTx.filter(tx => tx.fee_type === 'Tuition Fee').reduce((s, tx) => s + (Number(tx.paid_amount) || Number(tx.final_amount) || 0), 0));
      const admissionDue = Math.max(0, (st.agreed_admission_fee != null ? Number(st.agreed_admission_fee) : Number(config.admission_fee || 0)) - studentTx.filter(tx => tx.fee_type === 'Admission Fee').reduce((s, tx) => s + (Number(tx.paid_amount) || Number(tx.final_amount) || 0), 0));
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-slate-950 text-slate-100 font-sans">
      ...
      {/* UPDATED PVC ID CARD PRINT MODAL WITH BARCODE & QR */}
      {printIdCard && selectedIdStudent && (
        <div className="fixed inset-0 bg-slate-900/95 z-50 flex items-center justify-center p-4 pvc-modal-overlay">
          ...
          <div className="pvc-card w-[204px] h-[324px] bg-gradient-to-br from-slate-900 to-indigo-950 overflow-hidden relative flex flex-col text-white rounded-xl border-4 border-slate-800 shadow-2xl shrink-0">
             ...
             {/* Barcode placed safely at the bottom */}
              ...
              {selectedIdStudent && (
                <div className="pt-8 border-t border-slate-800 flex flex-col md:flex-row gap-10 items-center md:items-start">
                  ...
                  <div className="w-[204px] h-[324px] bg-gradient-to-br from-slate-900 to-indigo-950 overflow-hidden relative flex flex-col text-white rounded-xl border-4 border-slate-800 shadow-2xl shrink-0">
                     ...
                     {/* QR Code and Logo */}
                     {school.logo_url && <img src={school.logo_url} className="w-8 h-8 rounded-full absolute top-2 left-2 border border-slate-700 bg-white object-cover z-20" alt="Logo"/>}
                     <img src={`https://api.qrserver.com/v1/create-qr-code/?size=100x100&data=${encodeURIComponent(portalUrl)}`} className="w-8 h-8 absolute top-2 right-2 border border-slate-700 bg-white p-0.5 z-20 rounded-md" alt="QR Code"/>
