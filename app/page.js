...
export default function Dashboard() {
  ...
  const [studentSession, setStudentSession] = useState(null); // Local storage student
  ...
  // Added fatherName, dob, uniqueId
  const [formData, setFormData] = useState({
    id: null, name: '', rollNo: '', studentClass: 'Class 1', phone: '', bloodGroup: '', address: '', gender: 'Male', photoUrl: '', fatherName: '', dob: ''
  });
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      if (adminSession) {
        ...
        fetchSchoolDetails(); loadClassConfigs(); fetchData();
      } else if (localStudent) {
        const student = JSON.parse(localStudent);
        setStudentSession(student);
        fetchSchoolDetails(); loadClassConfigs();
  ...
  // Generate 99999 descending Unique ID
  ...
  const handleSaveStudent = async (e) => {
    ...
    if (isEditingStudent) {
      const { error } = await supabase.from('students').update({
        name: formData.name, father_name: formData.fatherName, dob: formData.dob, student_class: formData.studentClass, phone: formData.phone, blood_group: formData.bloodGroup, address: formData.address, gender: formData.gender, photo_url: formData.photoUrl || 'https://via.placeholder.com/150'
      }).eq('id', formData.id);
      ...
      const { error } = await supabase.from('students').insert([{
        name: formData.name, father_name: formData.fatherName, dob: formData.dob, unique_id: newUniqueId, roll_no: roll, student_class: formData.studentClass, phone: formData.phone, blood_group: formData.bloodGroup, address: formData.address, gender: formData.gender, status: 'Active', photo_url: formData.photoUrl || 'https://via.placeholder.com/150', email: `student_${formData.studentClass}_${roll}@school.com`
      }]);
  ...
  const handleEditClick = (st) => {
    setFormData({ id: st.id, name: st.name, fatherName: st.father_name || '', dob: st.dob || '', rollNo: st.roll_no, studentClass: st.student_class, phone: st.phone || '', bloodGroup: st.blood_group || '', address: st.address || '', gender: st.gender || 'Male', photoUrl: st.photo_url || '' });
  ...
  const handleChangeStatus = async () => {
    ...
    if (!error) { showToast(`স্টুডেন্ট ${statusAction} হিসেবে সেভ হয়েছে!`); setStatusModalStudent(null); fetchData(); } 
    else showToast('Error: ' + error.message, 'error');
    setIsProcessing(false);
  };

  const resetStudentForm = () => { setFormData({ id: null, name: '', fatherName: '', dob: '', rollNo: getNextRollForClass('Class 1'), studentClass: 'Class 1', phone: '', bloodGroup: '', address: '', gender: 'Male', photoUrl: '' }); setIsEditingStudent(false); };
  ...
  const handleSaveClassConfig = async () => {
    setIsProcessing(true);
    const { error } = await supabase.from('class_configs').upsert({ class_name: selectedConfigClass, academic_year: classConfig.academic_year, start_month: parseInt(classConfig.start_month) || 1, subjects: classConfig.subjects || [], admission_fee: classConfig.admission_fee, tuition_fee: classConfig.tuition_fee, exam1_fee: classConfig.exam1_fee, exam2_fee: classConfig.exam2_fee, exam3_fee: classConfig.exam3_fee, custom_fee: classConfig.custom_fee }, { onConflict: 'class_name' });
  ...
  const handleCreateInvoice = async () => {
    ...
    for (let i = 0; i < erpSelectedFeeTypes.length; i++) {
      ...
      const payable = feeType === 'Tuition Fee' ? (agreedFees.monthly * Math.max(1, (new Date().getMonth() + 1 - (erpClassConfig?.start_month || 1)) + 1)) : feeType === 'Admission Fee' ? agreedFees.admission : (erpClassConfig?.custom_fee||0);
  ...
  const calculateTotalDues = (selectedTypes, configData, aMonthly, aAdmission, studentTx = []) => {
    ...
    selectedTypes.forEach(feeType => {
      const payable = feeType === 'Tuition Fee' ? (aMonthly * Math.max(1, (new Date().getMonth() + 1 - (configData?.start_month || 1)) + 1)) : feeType === 'Admission Fee' ? aAdmission : (configData?.custom_fee||0);
  ...
  const getMonthlyStatus = (monthIndex, studentObj = null, txData = null, cConfig = null) => {
    ...
    const totalTuitionPaid = tx.filter(t => t.fee_type === 'Tuition Fee').reduce((s, t) => s + (Number(t.paid_amount) || Number(t.final_amount) || 0), 0);
    const feePerMonth = st?.agreed_monthly_fee !== null ? st?.agreed_monthly_fee : (config?.tuition_fee || 0);
  ...
  if (studentSession) {
    ...
    const aMonthly = studentSession.agreed_monthly_fee !== null ? studentSession.agreed_monthly_fee : (config.tuition_fee || 0);
    const aAdmission = studentSession.agreed_admission_fee !== null ? studentSession.agreed_admission_fee : (config.admission_fee || 0);

    const totalPaid = studentPortalData.tx.reduce((acc, curr) => acc + (Number(curr.paid_amount) || 0), 0);
    const currentMonth = new Date().getMonth() + 1;
    const activeMonths = Math.max(1, (currentMonth - (config.start_month || 1)) + 1);
    
    const expectedTuition = aMonthly * activeMonths;
    const tuitionPaid = studentPortalData.tx.filter(t => t.fee_type === 'Tuition Fee').reduce((s, t) => s + (Number(t.paid_amount) || Number(t.final_amount) || 0), 0);
    const admissionPaid = studentPortalData.tx.filter(t => t.fee_type === 'Admission Fee').reduce((s, t) => s + (Number(t.paid_amount) || Number(t.final_amount) || 0), 0);
        ...
        <div className="max-w-6xl mx-auto space-y-8">
          ...
          <div className="flex justify-between items-center bg-slate-900 border border-slate-800 p-6 rounded-3xl shadow-xl">
            <div className="flex items-center gap-4">
              <img src={school.logo_url || 'https://via.placeholder.com/50'} alt="Logo" className="w-14 h-14 rounded-full bg-white object-contain p-1 border-2 border-blue-500" />
              <div>
                <h1 className="text-xl md:text-2xl font-black text-white">{school.school_name || 'My School Portal'}</h1>
                <p className="text-xs text-blue-400 font-bold">Student Dashboard</p>
              </div>
            </div>
            <button onClick={handleLogout} className="bg-rose-500/10 text-rose-400 hover:bg-rose-500/20 px-5 py-2.5 rounded-xl font-bold flex items-center gap-2 transition">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-gradient-to-br from-blue-900 to-indigo-900 p-6 rounded-3xl shadow-xl border border-blue-500/30 flex items-center gap-5">
              ...
              <div>
                <h2 className="text-xl font-black text-white">{studentSession.name}</h2>
                <p className="text-sm text-blue-200 mt-1">{studentSession.student_class} | Roll: #{studentSession.roll_no}</p>
                <p className="text-xs font-bold bg-black/20 inline-block px-3 py-1 rounded-full mt-2">ID: {studentSession.unique_id}</p>
              </div>
            </div>
            <div className="bg-slate-900 border border-slate-800 p-6 rounded-3xl flex flex-col justify-center">
              <p className="text-xs text-slate-400 uppercase tracking-widest font-bold">Total Fees Paid</p>
              <h3 className="text-4xl font-black text-emerald-400 mt-2">₹{totalPaid}</h3>
            </div>
            <div className="bg-slate-900 border border-slate-800 p-6 rounded-3xl flex flex-col justify-center">
              <p className="text-xs text-slate-400 uppercase tracking-widest font-bold">Current Dues (Arrears)</p>
              <h3 className="text-4xl font-black text-rose-400 mt-2">₹{totalDue}</h3>
            </div>
          </div>
            ...
            <div className="bg-slate-900/60 border border-slate-800 p-6 rounded-3xl shadow-xl">
              ...
              <div className="grid grid-cols-3 md:grid-cols-4 gap-3">
                {monthsName.map((m, idx) => {
                  ...
                  return (
                    <div key={idx} className={`p-3 rounded-2xl text-center flex flex-col justify-center items-center h-20 border transition ${statusObj.bg} ${statusObj.border}`}>
      ...
      {printIdCard && selectedIdStudent && (
        <div className="fixed inset-0 bg-slate-900/95 z-50 flex items-center justify-center p-4 pvc-modal-overlay">
          <div className="absolute top-6 right-6 flex gap-4 no-print">
            ...
            <button onClick={() => window.print()} className="bg-blue-600 px-5 py-2.5 rounded-xl font-bold text-white shadow-lg shadow-blue-500/30 hover:bg-blue-500 transition"><Printer size={16} className="inline mr-2" /> Print PVC</button>
          </div>
          <div className="pvc-card w-[204px] h-[324px] bg-gradient-to-br from-blue-900 to-indigo-950 overflow-hidden relative flex flex-col items-center justify-between text-white p-4 rounded-xl border-4 border-slate-800 shadow-2xl">
             ...
             <div className="text-center w-full z-10">
                 ...
                 <p className="text-[9px] text-white mt-1">ID: {selectedIdStudent.unique_id}</p>
             </div>
             <div className="bg-slate-950/60 backdrop-blur-md w-full rounded-lg p-2.5 text-[9px] space-y-1.5 mt-1.5 z-10 border border-slate-700">
                 <p><strong>DOB:</strong> <span className="text-white">{selectedIdStudent.dob || 'N/A'}</span></p>
            ...
            <div className="bg-slate-900/60 border border-slate-800/80 p-6 md:p-8 rounded-2xl shadow-xl">
              ...
              <form onSubmit={handleSaveStudent} className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  ...
                  <div><label className="text-xs font-semibold text-slate-300 mb-2 flex items-center gap-2"><User size={14}/> স্টুডেন্টের নাম *</label><input type="text" name="name" value={formData.name} onChange={handleInputChange} className="w-full bg-slate-950 border border-slate-800 px-4 py-3 rounded-xl text-white outline-none focus:border-blue-500" required /></div>
                  <div><label className="text-xs font-semibold text-slate-300 mb-2 flex items-center gap-2"><User size={14}/> পিতার নাম (Father's Name)</label><input type="text" name="fatherName" value={formData.fatherName} onChange={handleInputChange} className="w-full bg-slate-950 border border-slate-800 px-4 py-3 rounded-xl text-white outline-none focus:border-blue-500" /></div>
                  <div><label className="text-xs font-semibold text-slate-300 mb-2 flex items-center gap-2"><Calendar size={14}/> জন্মতারিখ (DOB) *</label><input type="date" name="dob" value={formData.dob} onChange={handleInputChange} className="w-full bg-slate-950 border border-slate-800 px-4 py-3 rounded-xl text-white outline-none focus:border-blue-500" required /></div>
                  <div><label className="text-xs font-semibold text-slate-300 mb-2 flex items-center gap-2"><Phone size={14}/> মোবাইল নম্বর</label><input type="text" name="phone" value={formData.phone} onChange={handleInputChange} className="w-full bg-slate-950 border border-slate-800 px-4 py-3 rounded-xl text-white outline-none focus:border-blue-500" /></div>
                  <div>
                    ...
                    {getFilteredStudents().map((st) => (
                      <tr key={st.id} className="hover:bg-slate-800/30 transition">
                        ...
                        <td className="p-4 flex gap-2 justify-center">
                          ...
                          {/* UNIVERSAL DELETE (AVAILABLE EVERYWHERE) */}
