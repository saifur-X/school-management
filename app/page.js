...
export default function Dashboard() {
  ...
  const [erpSelectedClass, setErpSelectedClass] = useState(''); // NEW: Class Filter for ERP
  ...
  // Helper to dynamically get classes that actually have students
  const getClassesWithStudents = () => {
    const uniqueClasses = [...new Set(students.map(s => s.student_class))];
    return uniqueClasses.map(className => ({
      className,
      count: students.filter(s => s.student_class === className).length
  ...
  const handleUpdateTx = async () => {
    ...
    if (!error) {
      ...
      alert('আপডেট ফেইলড: ' + error.message);
    }
  };
  ...
  // Calculate Global Dues Fix (Ensuring table and top cards exactly match)
  ...
  const generatePendingDuesArray = () => {
    ...
    students.forEach(st => {
      ...
      if (tuitionDue > 0) {
        pendingList.push({ id: st.id + '_tui', students: st, fee_type: 'Tuition Fee (Pending)', pending_amount: tuitionDue });
      }
      if (admissionDue > 0) {
        pendingList.push({ id: st.id + '_adm', students: st, fee_type: 'Admission Fee (Pending)', pending_amount: admissionDue });
      }
    });
    return pendingList;
  };
    ...
    <div className="flex flex-col md:flex-row min-h-screen bg-slate-950 text-slate-100 font-sans">
      ...
      {receiptData && (
        <div className="fixed inset-0 bg-white text-slate-900 z-50 p-6 overflow-y-auto print:p-0">
          <style jsx global>{`@media print { @page { size: A4 portrait; margin: 10mm; } body { background: white !important; color: black !important; } .no-print { display: none !important; } }`}</style>
              ...
              <form onSubmit={handleSaveStudent} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  <div>
                    ...
                    {!isAddingCustomClass ? (<select name="studentClass" value={formData.studentClass} onChange={handleClassChangeInForm} className="w-full bg-slate-950 border border-slate-800 px-4 py-3 rounded-xl text-white">{classList.map(cls => <option key={cls} value={cls}>{cls}</option>)}<option value="CUSTOM">+ Custom Class</option></select>) : (<div className="flex gap-2"><input type="text" placeholder="Class Name" value={customClassInput} onChange={(e) => setCustomClassInput(e.target.value)} className="bg-slate-950 border border-slate-800 px-3 py-3 rounded-xl text-white flex-1 text-xs" /><button type="button" onClick={handleAddCustomClass} className="bg-blue-600 px-4 py-2 rounded-xl text-xs font-bold">Save</button></div>)}
                  </div>
          ...
          <div className="space-y-6 animate-fade-in">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              <div className="flex gap-3">
                <select value={selectedClassFilter} onChange={(e) => setSelectedClassFilter(e.target.value)} className="bg-slate-900 border border-slate-800 px-4 py-2 rounded-xl text-sm"><option value="All">সকল ক্লাস</option>{classList.map(cls => <option key={cls} value={cls}>{cls}</option>)}</select>
                {selectedClassFilter !== 'All' && (<button onClick={() => handleUpgradeClass(selectedClassFilter)} className="bg-emerald-600 hover:bg-emerald-700 px-4 py-2 rounded-xl text-xs font-bold flex items-center gap-1.5"><ArrowUpCircle size={16} /> {selectedClassFilter} আপগ্রেড করুন</button>)}
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                  ...
                  <div className="bg-slate-900/60 border border-slate-800 p-6 rounded-2xl shadow-xl">
                    <div className="space-y-4">
                      <div>
                        ...
                        <select value={erpSelectedClass} onChange={(e) => { setErpSelectedClass(e.target.value); setErpStudent(null); }} className="w-full bg-slate-950 border border-slate-800 p-3 rounded-xl text-white text-sm font-bold">
                          <option value="">ক্লাস বেছে নিন...</option>
                          {getClassesWithStudents().map(c => (
                            <option key={c.className} value={c.className}>{c.className} ({c.count} Students)</option>
                          ))}
                        </select>
                      </div>
                ...
                {erpStudent && erpClassConfig && (
                  <div className="lg:col-span-8 space-y-6">
                    ...
                    {/* Transaction History With Print & Edit Buttons */}
          ...
          <div className="space-y-6 animate-fade-in">
            ...
            {selectedIdStudent && (
              <div className="w-80 bg-gradient-to-br from-indigo-950 via-slate-900 to-blue-950 border border-indigo-500/30 p-6 rounded-3xl shadow-2xl text-center border-t-4 border-t-blue-500">
                <h3 className="font-extrabold text-blue-300 text-lg">{school.school_name}</h3><p className="text-[10px] text-slate-400">{school.address}</p>
                ...
                <h4 className="font-bold text-xl text-white">{selectedIdStudent.name}</h4><p className="text-xs text-blue-400 font-semibold mb-4">{selectedIdStudent.student_class} | Roll: #{selectedIdStudent.roll_no}</p>
                <div className="text-left text-xs space-y-1.5 bg-slate-950/70 p-3.5 rounded-2xl border border-slate-800/80 text-slate-300"><p><strong>Blood:</strong> <span className="text-rose-400 font-bold">{selectedIdStudent.blood_group || 'N/A'}</span></p><p><strong>Phone:</strong> {selectedIdStudent.phone || 'N/A'}</p><p><strong>Address:</strong> {selectedIdStudent.address || 'N/A'}</p></div>
              </div>
            )}
          </div>
        )}
        ...
        {activeTab === 'profile' && (
          <div className="space-y-6 animate-fade-in">
            <div className="flex justify-between items-center">
              ...
              <button onClick={() => setEditSchool(!editSchool)} className="bg-slate-800 px-4 py-2 rounded-xl text-xs font-semibold">{editSchool ? 'বাতিল' : 'এডিট করুন'}</button>
            </div>
