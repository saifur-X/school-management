...
export default function Dashboard() {
  ...
  const loadClassConfigs = async () => {
    ...
    if (data) {
      const configMap = {};
      data.forEach(item => {
        configMap[item.class_name] = item;
      });
  ...
  const handleSaveClassConfig = async () => {
    ...
    if (error) alert('সেভ করা যায়নি: ' + error.message);
      ...
      alert('Class Settings & Academic Config সেভ হয়েছে!');
      loadClassConfigs();
    }
  };
  ...
  // Fee calculation with Already Paid checking logic
  const updateBaseFeeBySelection = (feeType, configData, studentTx = []) => {
    if (!configData) return;
    setErpFeeType(feeType);

    // Calculate how much is already paid for this specific fee type
    const alreadyPaid = studentTx
      .filter(tx => tx.fee_type === feeType)
      .reduce((sum, tx) => sum + (Number(tx.paid_amount) || Number(tx.final_amount) || 0), 0);
    ...
    if (feeType === 'Tuition Fee') {
      ...
      fullFee = (configData.tuition_fee || 0) * activeMonthsCount;
    } else if (feeType === 'Admission Fee') fullFee = configData.admission_fee || 0;
    else if (feeType === 'Term 1 Exam Fee') fullFee = configData.exam1_fee || 0;
    else if (feeType === 'Term 2 Exam Fee') fullFee = configData.exam2_fee || 0;
    else if (feeType === 'Term 3 Exam Fee') fullFee = configData.exam3_fee || 0;
    else if (feeType === 'Custom Fee') fullFee = configData.custom_fee || 0;

    const remainingDue = Math.max(0, fullFee - alreadyPaid);
    setErpBaseAmount(remainingDue);
  };
  ...
  const handleSelectErpStudent = async (st) => {
    ...
    if (configData) {
      updateBaseFeeBySelection('Tuition Fee', configData, currentTx);
    }
  };
  ...
  // Global Total Pending Calculation including non-paid expected fees
  ...
  const totalPendingDue = students.reduce((acc, st) => {
    ...
    // Monthly Tuition Expected
    const currentMonth = new Date().getMonth() + 1;
    const startM = config.start_month || 1;
    const activeMonths = Math.max(1, (currentMonth - startM) + 1);
    const expectedTuition = (config.tuition_fee || 0) * activeMonths;
    const expectedAdmission = config.admission_fee || 0;

    const studentTx = allErpTransactions.filter(tx => tx.student_id === st.id);
    const tuitionPaid = studentTx.filter(tx => tx.fee_type === 'Tuition Fee').reduce((s, tx) => s + (Number(tx.paid_amount) || 0), 0);
                      ...
                      <div>
                        ...
                        <select 
                          ...
                          onChange={async (e) => {
                            ...
                            updateBaseFeeBySelection(type, cData, erpTransactions);
                          }} 
