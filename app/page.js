...
export default function Dashboard() {
  ...
  const [agreedFees, setAgreedFees] = useState({ monthly: 0, admission: 0 }); // Student specific fixed fees
  
  const [erpFeeType, setErpFeeType] = useState('Admission Fee');
  ...
  const handleEditClick = (st) => {
    setFormData({ 
      ...
      phone: st.phone || '', bloodGroup: st.blood_group || '', address: st.address || '', 
      gender: st.gender || 'Male', photoUrl: st.photo_url || '' 
    });
  ...
  const handleSelectErpStudent = async (st) => {
    ...
    updateBaseFeeBySelection('Admission Fee', cConfig, aMonthly, aAdmission, currentTx);
  };

  const saveAgreedFeesToDB = async () => {
    if (!erpStudent) return;
    const { error } = await supabase.from('students').update({
      agreed_monthly_fee: agreedFees.monthly,
      agreed_admission_fee: agreedFees.admission
    }).eq('id', erpStudent.id);
    
    if (!error) {
      alert("এই স্টুডেন্টের নির্দিষ্ট ফিস প্রোফাইল সেভ হয়েছে!");
      fetchStudents(); 
      updateBaseFeeBySelection(erpFeeType, erpClassConfig, agreedFees.monthly, agreedFees.admission, erpTransactions);
    }
  };

  const updateBaseFeeBySelection = (feeType, configData, aMonthly, aAdmission, studentTx = []) => {
    ...
    if (feeType.startsWith('Tuition Fee')) {
      base = configData.tuition_fee || 0;
      payable = aMonthly;
    } else if (feeType === 'Admission Fee') {
      base = configData.admission_fee || 0;
      payable = aAdmission;
    } else if (feeType === 'Term 1 Exam Fee') { base = configData.exam1_fee || 0; payable = base; }
      else if (feeType === 'Term 2 Exam Fee') { base = configData.exam2_fee || 0; payable = base; }
      else if (feeType === 'Term 3 Exam Fee') { base = configData.exam3_fee || 0; payable = base; }
      else { base = configData.custom_fee || 0; payable = base; }

    const autoDiscount = Math.max(0, base - payable);
    const alreadyPaid = studentTx.filter(tx => tx.fee_type === feeType).reduce((s, tx) => s + (Number(tx.paid_amount) || 0), 0);
    
    setErpBaseAmount(base);
    setErpDiscount(autoDiscount);
    
    if (alreadyPaid >= payable && payable > 0) {
      setErpPaidAmount(0); 
    } else {
      setErpPaidAmount(payable - alreadyPaid); 
    }
  };
  ...
  const handleCreateInvoice = async () => {
    ...
    const alreadyPaidBefore = erpTransactions.filter(tx => tx.fee_type === erpFeeType).reduce((s, tx) => s + (Number(tx.paid_amount) || 0), 0);
    const totalPaidNow = alreadyPaidBefore + paid;
    const pending = Math.max(0, finalPayable - totalPaidNow);
    const status = pending <= 0 ? 'Paid' : 'Pending';

    if (paid <= 0 && pending > 0) return alert('Paid Amount 0 হতে পারে না!');
  ...
  const getMonthlyStatus = (monthIndex) => {
    ...
    if (monthIndex < startM) return { text: 'N/A', color: 'bg-slate-900 text-slate-600 border border-slate-800' };
    
    const feeName = `Tuition Fee - ${monthsName[monthIndex]}`;
    const tx = erpTransactions.find(t => t.fee_type === feeName);
    
    if (tx && tx.status === 'Paid') return { text: 'Paid', color: 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/30' };
    if (tx && tx.status === 'Pending') return { text: `Due (₹${tx.pending_amount})`, color: 'bg-rose-500/10 text-rose-400 border border-rose-500/30' };
    
    if (monthIndex <= currentMonth) return { text: 'Due', color: 'bg-rose-500/10 text-rose-400 border border-rose-500/30' };
    return { text: 'Upcoming', color: 'bg-blue-500/10 text-blue-400 border border-blue-500/30' };
  };
  ...
  const totalPendingDue = students.reduce((acc, st) => {
    ...
    const tuitionPaid = studentTx.filter(tx => tx.fee_type.startsWith('Tuition Fee')).reduce((s, tx) => s + (Number(tx.paid_amount) || 0), 0);
    const admissionPaid = studentTx.filter(tx => tx.fee_type === 'Admission Fee').reduce((s, tx) => s + (Number(tx.paid_amount) || 0), 0);
                ...
                {erpStudent && erpClassConfig && (
                  <div className="lg:col-span-8 space-y-6">
                    ...
                    <div className="bg-slate-900/60 border border-slate-800 p-6 rounded-2xl shadow-xl">
                      ...
                      <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
                        {monthsName.map((m, idx) => {
                          ...
                          return (
                            <div key={idx} className={`p-3 rounded-xl text-center flex flex-col justify-center items-center h-20 ${statusObj.color}`}>
                              <span className="text-xs font-bold">{m}</span>
                              <span className="text-[10px] mt-1 font-semibold bg-slate-950/50 px-2 py-0.5 rounded">{statusObj.text}</span>
                            </div>
                          );
                        })}
                      </div>
                    ...
                    <div className="bg-slate-900/60 border border-slate-800 p-6 rounded-2xl shadow-xl space-y-4">
                      <h3 className="text-lg font-bold text-emerald-400">ফি কালেকশন ও রসিদ</h3>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          ...
                          <select 
                            value={erpFeeType} 
                            onChange={(e) => updateBaseFeeBySelection(e.target.value, erpClassConfig, agreedFees.monthly, agreedFees.admission, erpTransactions)} 
                            className="w-full bg-slate-950 border border-slate-800 p-3 rounded-xl text-white text-xs"
                          >
                            <option value="Admission Fee">Admission Fee</option>
                            {monthsName.map(m => <option key={m} value={`Tuition Fee - ${m}`}>Tuition Fee - {m}</option>)}
                        ...
                        <div>
                          <label className="text-xs text-slate-400 block mb-1">Payable Amount Now (₹)</label>
                          <input type="number" value={erpPaidAmount} onChange={(e) => setErpPaidAmount(e.target.value)} className="w-full bg-slate-950 border border-slate-800 p-3 rounded-xl text-emerald-400 font-bold text-sm" />
                        </div>
                      </div>

                      <div className="p-4 bg-slate-950 rounded-xl space-y-1 text-xs border border-slate-800">
                        <div className="flex justify-between text-slate-500">
                          <span>Original Class Base Fee:</span> <span>₹{erpBaseAmount}</span>
                        </div>
                        <div className="flex justify-between text-blue-400">
                          <span>Auto Calculated Discount:</span> <span>-₹{erpDiscount}</span>
                        </div>
                        <div className="flex justify-between text-rose-400 font-bold pt-2 border-t border-slate-800 mt-2">
                          <span>Remaining Pending/Due:</span> 
                          <span>₹{Math.max(0, (Number(erpBaseAmount) - Number(erpDiscount)) - Number(erpPaidAmount))}</span>
                        </div>
                      </div>
