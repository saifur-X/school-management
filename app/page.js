...
export default function Dashboard() {
  ...
  // ==============================
  // CLASS CONFIG & FIX FOR RENDER CRASH
  // ==============================
  const fetchClassConfigDetails = async (cls) => {
    ...
    if (data) { 
      setClassConfig({ 
        ...
        // Safety Fallback for empty subjects
        subjects: data.subjects && data.subjects.length > 0 ? data.subjects : [{ name: 'Bangla', oral: 20, theory: 80 }, { name: 'English', oral: 20, theory: 80 }]
      }); 
    } else { 
      setClassConfig({ 
        academic_year: '2026', start_month: 1, 
  ...
  const handleSaveClassConfig = async () => {
    ...
    const { error } = await supabase.from('class_configs').upsert({ 
      class_name: selectedConfigClass, academic_year: classConfig.academic_year, start_month: parseInt(classConfig.start_month) || 1, 
      subjects: classConfig.subjects || [], admission_fee: classConfig.admission_fee, tuition_fee: classConfig.tuition_fee, 
      exam1_fee: classConfig.exam1_fee, exam2_fee: classConfig.exam2_fee, exam3_fee: classConfig.exam3_fee, custom_fee: classConfig.custom_fee 
