...
export default function Dashboard() {
  ...
  const [formData, setFormData] = useState({
    ...
    fatherName: '', motherName: '', dob: '', aadharNo: '', religion: '', category: 'General'
  });
  ...
  // === UPDATED HANDLE EDIT CLICK ===
  const handleEditClick = (st) => {
    setFormData({ 
      id: st.id, name: st.name, fatherName: st.father_name || '', motherName: st.mother_name || '', dob: st.dob || '', 
      rollNo: st.roll_no, studentClass: st.student_class, phone: st.phone || '', bloodGroup: st.blood_group || '', 
      address: st.address || '', gender: st.gender || 'Male', photoUrl: st.photo_url || '',
      aadharNo: st.aadhar_no || '', religion: st.religion || '', category: st.category || 'General'
    });
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-slate-950 text-slate-100 font-sans">
      ...
      {/* ==============================================
          NEW ADMISSION FORM MODAL (POPUP)
          ============================================== */}
            ...
            <form onSubmit={handleSaveStudent} className="space-y-6">
              <div>
                ...
                <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                  {classList.map(cls => (
                    <button type="button" key={cls} onClick={() => handleClassChangeInForm({target: {value: cls}})} className={`px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap transition ${formData.studentClass === cls ? 'bg-blue-600 text-white shadow-lg' : 'bg-slate-950 text-slate-400 border border-slate-800 hover:bg-slate-800'}`}>{cls}</button>
                  ))}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div><label className="text-xs font-semibold text-slate-300 mb-2 flex items-center gap-2"><User size={14}/> মাতার নাম (Mother's Name)</label><input type="text" name="motherName" value={formData.motherName} onChange={handleInputChange} className="w-full bg-slate-950 border border-slate-800 px-4 py-3 rounded-xl text-white outline-none focus:border-blue-500" /></div>
                ...
                {/* NEW FIELDS */}
                <div><label className="text-xs font-semibold text-slate-300 mb-2 block">Aadhar / National ID</label><input type="text" name="aadharNo" value={formData.aadharNo} onChange={handleInputChange} className="w-full bg-slate-950 border border-slate-800 px-4 py-3 rounded-xl text-white outline-none focus:border-blue-500" /></div>
                <div><label className="text-xs font-semibold text-slate-300 mb-2 block">Religion</label><input type="text" name="religion" value={formData.religion} onChange={handleInputChange} className="w-full bg-slate-950 border border-slate-800 px-4 py-3 rounded-xl text-white outline-none focus:border-blue-500" /></div>
                <div><label className="text-xs font-semibold text-slate-300 mb-2 block">Category</label><select name="category" value={formData.category} onChange={handleInputChange} className="w-full bg-slate-950 border border-slate-800 px-4 py-3 rounded-xl text-white outline-none focus:border-blue-500"><option value="General">General</option><option value="OBC">OBC</option><option value="SC">SC</option><option value="ST">ST</option></select></div>
              ...
              <button type="submit" disabled={isProcessing} className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 px-8 py-3.5 rounded-xl font-bold shadow-lg shadow-blue-500/20 flex justify-center items-center gap-2 transition text-white">
                {isProcessing ? <Loader2 className="animate-spin" size={18}/> : <Save size={18}/>} 
                {isEditingStudent ? 'Update Profile' : 'Submit Admission'}
              </button>
            </form>
          </div>
        </div>
      ...
      <main className="flex-1 p-6 md:p-10 overflow-y-auto w-full">
        ...
        {/* DASHBOARD TAB WITH ADVANCED ANALYTICS (MODIFIED) */}
        {activeTab === 'dashboard' && (
          <div className="space-y-8 animate-fade-in">
            {/* TOP HEADER WITH + ADMISSION BUTTON */}
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-slate-800 pb-4 gap-4">
               <div>
                  <h2 className="text-3xl font-bold text-white">Dashboard & Analytics</h2>
                  <p className="text-sm text-slate-400 mt-1">Welcome back to your school management portal.</p>
               </div>
               
               {/* NEW ADMISSION BUTTON TRIGGER */}
               <button onClick={() => setIsAdmissionModalOpen(true)} className="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 px-6 py-3 rounded-xl font-bold text-white shadow-lg shadow-blue-500/30 flex items-center gap-2 transition scale-100 hover:scale-105">
                 <Plus size={20} className="font-black"/> New Admission
               </button>
            </div>
            ...
            {/* DASHBOARD EXTRA DETAILS (Since Form is moved to Modal) */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {/* Quick Actions / Notices Placeholder */}
              <div className="bg-slate-900/60 border border-slate-800/80 p-6 rounded-2xl shadow-xl">
                 <h3 className="text-lg font-bold text-amber-400 flex items-center gap-2 mb-4"><Bell size={18}/> Recent Activities & Notices</h3>
                 <div className="space-y-4">
                    <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 text-sm">
                       <p className="text-slate-300 font-medium">Exam schedules for Term 1 have been published.</p>
                       <p className="text-xs text-slate-500 mt-1">Just now</p>
                    </div>
                    <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 text-sm">
                       <p className="text-slate-300 font-medium">New student admitted: Rahul Das (Class 5)</p>
                       <p className="text-xs text-slate-500 mt-1">2 hours ago</p>
                    </div>
                    <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 text-sm">
                       <p className="text-slate-300 font-medium">System maintenance completed successfully.</p>
              ...
              <div className="bg-slate-900/60 border border-slate-800/80 p-6 rounded-2xl shadow-xl">
                 <h3 className="text-lg font-bold text-emerald-400 flex items-center gap-2 mb-4"><Activity size={18}/> Class-wise Enrollment Stats</h3>
                 <div className="space-y-3">
                   {activeClasses.slice(0, 5).map(cls => {
                     const clsCount = students.filter(s => s.student_class === cls && (s.status === 'Active' || !s.status)).length;
                     return (
                       <div key={cls} className="flex justify-between items-center bg-slate-950 p-3 rounded-xl border border-slate-800">
                         <span className="text-sm font-bold text-slate-300">{cls}</span>
                         <span className="bg-blue-500/20 text-blue-400 px-3 py-1 rounded-lg text-xs font-black">{clsCount} Students</span>
                       </div>
                     )
                   })}
                   {activeClasses.length === 0 && <p className="text-sm text-slate-500">No active classes found.</p>}
                 </div>
              </div>
            </div>
          </div>
