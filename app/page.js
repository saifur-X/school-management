...
export default function Dashboard() {
  ...
  const handleUpgradeClass = (currentClass) => {
    const nextClassMap = { 'Class 1':'Class 2', 'Class 2':'Class 3', 'Class 3':'Class 4', 'Class 4':'Class 5', 'Class 5':'Class 6', 'Class 6':'Class 7', 'Class 7':'Class 8', 'Class 8':'Class 9', 'Class 9':'Class 10', 'Class 10':'Class 11', 'Class 11':'Class 12', 'Class 12':'Graduated' };
  ...
  const handleUpdateSchool = async (e) => {
    ...
    if (!error) { alert('স্কুলের তথ্য আপডেট হয়েছে!'); setEditSchool(false); }
  };
  ...
  const fetchClassConfigDetails = async (cls) => {
    ...
    if (data) {
      setClassConfig({ ...data, academic_year: data.academic_year || '2026', start_month: data.start_month || 1 });
    } else {
      setClassConfig({ academic_year: '2026', start_month: 1, subjects: [{ name: 'Bangla', oral: 20, theory: 80 }, { name: 'English', oral: 20, theory: 80 }], admission_fee: 1000, tuition_fee: 500, exam1_fee: 200, exam2_fee: 200, exam3_fee: 200, custom_fee: 0 });
    }
  };
  ...
  const handleSelectErpStudent = async (st) => {
    ...
    calculateTotalDues(erpSelectedFeeTypes, cConfig, aMonthly, aAdmission, currentTx);
  };

  const saveAgreedFeesToDB = async () => {
    if (!erpStudent) return;
    const { error } = await supabase.from('students').update({ agreed_monthly_fee: agreedFees.monthly, agreed_admission_fee: agreedFees.admission }).eq('id', erpStudent.id);
    if (!error) {
      alert("এই স্টুডেন্টের নির্দিষ্ট ফিস প্রোফাইল সেভ হয়েছে!");
      fetchStudents(); 
      calculateTotalDues(erpSelectedFeeTypes, erpClassConfig, agreedFees.monthly, agreedFees.admission, erpTransactions);
    }
  };

  const toggleFeeType = (feeType) => {
    let updatedTypes = [...erpSelectedFeeTypes];
    if (updatedTypes.includes(feeType)) {
      updatedTypes = updatedTypes.filter(t => t !== feeType);
    } else {
      updatedTypes.push(feeType);
    }
    if (updatedTypes.length === 0) updatedTypes = ['Tuition Fee']; // keep at least one
  ...
  const calculateTotalDues = (selectedTypes, configData, aMonthly, aAdmission, studentTx = []) => {
    ...
    setErpPaidAmount(totalDue); // Suggest paying full due
    setErpDiscount(0); // Discount is manually entered now
  };
  ...
  const handleCreateInvoice = async () => {
    ...
    // Process each selected fee type sequentially (Wallet Allocation Logic)
    for (let i = 0; i < erpSelectedFeeTypes.length; i++) {
      ...
      // If it's the last selected item OR it's Tuition Fee, dump all remaining money into it (Advance payment logic)
      ...
      // Only insert if there's money to allocate or if they applied a discount
      if (allocateAmount > 0 || (!discountApplied && globalDiscount > 0)) {
        let appliedDisc = 0;
        if (!discountApplied && globalDiscount > 0) {
          appliedDisc = globalDiscount;
        ...
        transactionsToInsert.push({
          ...
          final_amount: netPayableForThisItem,
          paid_amount: allocateAmount,
          pending_amount: itemPending,
          status: (newTotalPaidForThisItem >= netPayableForThisItem) ? 'Paid' : 'Pending'
        });
    ...
    if (error) {
      ...
      setReceiptData({
        ...
        student: erpStudent,
        items: receiptItems,
        totalPaid: Number(erpPaidAmount)
      });
  ...
  // Sequential Logic for Monthly Grid UI
  const getMonthlyStatus = (monthIndex) => {
    ...
    // Total Tuition paid over all time
    ...
    // Figure out how much of the wallet is used up BEFORE this month
    ...
    if (availableForThisMonth >= feePerMonth) {
      ...
      const dueLeft = feePerMonth - availableForThisMonth;
      return { label: 'Partially Paid', subText: `Paid: ₹${availableForThisMonth}`, bg: 'bg-lime-500/10', text: 'text-lime-400', border: 'border-lime-500/40', badgeBg: 'bg-lime-500/20' };
    } else {
  ...
  const totalPendingDue = students.reduce((acc, st) => {
    ...
    return acc + tuitionDue + admissionDue;
  }, 0);

  const pendingTransactionsList = students.map(st => {
    const config = allClassConfigs[st.student_class];
    ...
    const due = Math.max(0, expectedTuition - tuitionPaid);
    if (due > 0) {
      return { id: st.id, students: st, fee_type: 'Tuition Fee (Pending)', pending_amount: due };
    }
    return null;
  }).filter(Boolean);
          ...
          <div className="bg-slate-900 border border-slate-800 p-6 rounded-3xl max-w-sm w-full space-y-4 shadow-2xl relative">
            ...
            <div className="space-y-3 bg-slate-950 p-4 rounded-2xl border border-slate-800 text-xs text-slate-300">
              <div className="flex items-center gap-3">
                ...
                <div><p className="text-[10px] text-slate-500">Contact Number</p><p className="font-bold text-white">{contactModalStudent.phone || 'N/A'}</p></div>
              </div>
              <div className="flex items-center gap-3">
                <Droplet className="text-rose-400" size={16} />
                <div><p className="text-[10px] text-slate-500">Blood Group</p><p className="font-bold text-rose-400">{contactModalStudent.blood_group || 'N/A'}</p></div>
              </div>
              <div className="flex items-center gap-3">
                <MapPin className="text-emerald-400" size={16} />
                <div><p className="text-[10px] text-slate-500">Address</p><p className="font-medium">{contactModalStudent.address || 'N/A'}</p></div>
              </div>
            </div>
            ...
            <table className="w-full text-left text-xs border-collapse border border-slate-900 mb-4">
              <thead>
                <tr className="bg-slate-200 border-b border-slate-900">
                  <th className="p-2 border-r border-slate-900">Fee Category</th>
                  <th className="p-2 text-right border-r border-slate-900">Net Amount</th>
              ...
              <tbody>
                ...
                <tr className="bg-slate-100 font-bold border-b border-slate-900">
                  <td colSpan="2" className="p-2 border-r border-slate-900 text-right">Total Amount Paid Now:</td>
                  <td className="p-2 text-right text-blue-900 text-sm font-black">₹{receiptData.totalPaid}</td>
                </tr>
              </tbody>
            </table>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  <div>
                    <label className="text-xs font-semibold text-slate-300 mb-2 flex items-center gap-2">
                      <BookOpen size={14}/> ক্লাস সিলেক্ট করুন *
                    </label>
                    {!isAddingCustomClass ? (
                  ...
                  <div className="md:col-span-2">
                    <label className="text-xs font-semibold text-slate-300 mb-2 flex items-center gap-2">
                      <Image size={14}/> ফটো ইমেজ URL (Optional)
                    </label>
            ...
            {!showOnlyPendingList ? (
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                  ...
                  <div className="bg-slate-900/60 border border-slate-800 p-6 rounded-2xl shadow-xl">
                    ...
                    {erpStudent && (
                      <div className="mt-6 pt-6 border-t border-slate-800 space-y-4">
                        ...
                        <div>
                          <label className="text-xs text-slate-400 block mb-1">Agreed Monthly Fee (₹)</label>
                          <input type="number" value={agreedFees.monthly} onChange={e => setAgreedFees({...agreedFees, monthly: e.target.value})} className="w-full bg-slate-950 border border-slate-800 p-2.5 rounded-xl text-white text-sm" />
                        </div>
                        <div>
                          <label className="text-xs text-slate-400 block mb-1">Agreed Admission Fee (₹)</label>
                          <input type="number" value={agreedFees.admission} onChange={e => setAgreedFees({...agreedFees, admission: e.target.value})} className="w-full bg-slate-950 border border-slate-800 p-2.5 rounded-xl text-white text-sm" />
                        </div>
                        <button onClick={saveAgreedFeesToDB} className="w-full bg-amber-600/20 text-amber-500 border border-amber-500/30 py-2 rounded-xl text-xs font-bold hover:bg-amber-600/40 transition">
                          প্রোফাইল সেভ করুন
                        </button>
                      </div>
                    )}
                ...
                {erpStudent && erpClassConfig && (
                  <div className="lg:col-span-8 space-y-6">
                    ...
                    {/* Monthly Tracker Grid (Wallet Engine) */}
                    <div className="bg-slate-900/60 border border-slate-800 p-6 rounded-2xl shadow-xl">
                      <h3 className="text-lg font-bold text-blue-400 mb-4 flex items-center gap-2"><Calendar size={18}/> Monthly Status Tracker ({erpClassConfig.academic_year})</h3>
                    ...
                    {/* Multiple Fee Selection & Payment Form */}
                    <div className="bg-slate-900/60 border border-slate-800 p-6 rounded-2xl shadow-xl space-y-5">
                      <h3 className="text-lg font-bold text-emerald-400">ফি কালেকশন সার্ভিস (Multi-Select)</h3>
                          ...
                          {feeOptionsList.map(fee => (
                            <button
                              ...
                              className={`px-4 py-2 rounded-xl text-xs font-bold border transition ${
                                erpSelectedFeeTypes.includes(fee) 
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                          ...
                          <input type="number" placeholder="Enter manual discount" value={erpDiscount} onChange={(e) => setErpDiscount(e.target.value)} className="w-full bg-slate-950 border border-slate-800 p-3 rounded-xl text-white text-sm focus:border-blue-500 outline-none" />
                        </div>
                      </div>

                      <div>
                        <label className="text-xs text-emerald-400 block mb-1">Amount Paying Now (₹) *</label>
                        <input type="number" placeholder="কত টাকা প্রদান করল" value={erpPaidAmount} onChange={(e) => setErpPaidAmount(e.target.value)} className="w-full bg-slate-950 border border-emerald-500/50 p-4 rounded-xl text-emerald-400 font-black text-lg focus:border-emerald-400 outline-none" />
                        <p className="text-[10px] text-slate-500 mt-2">💡 Tuition Fee-র ক্ষেত্রে কম টাকা দিলে মাসগুলো ক্রমানুসারে আংশিক ফিল হবে এবং বেশি দিলে অ্যাডভান্স হিসেবে সামনের মাসে ফিল হয়ে যাবে।</p>
                      </div>

                      <button onClick={handleCreateInvoice} className="w-full bg-gradient-to-r from-emerald-600 to-emerald-500 hover:from-emerald-500 hover:to-emerald-400 text-white py-3.5 rounded-xl font-black text-sm shadow-lg shadow-emerald-500/20 transition transform hover:scale-[1.01]">
                        ফি জমা নিন ও রসিদ প্রিন্ট করুন
                      </button>
                    </div>

                    {/* Transaction History Restoration */}
                          ...
                          <table className="w-full text-left text-xs border-collapse">
                            <thead className="bg-slate-800 text-slate-400">
                              <tr>
                                ...
                                <th className="p-3">ফি টাইপ</th>
                                <th className="p-3 text-right">Payment</th>
                                <th className="p-3 text-right">Due Left</th>
                              </tr>
                            </thead>
              ...
              <div className="bg-slate-900/60 border border-slate-800 p-6 rounded-2xl space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <button onClick={() => setShowOnlyPendingList(false)} className="bg-slate-800 px-3 py-1.5 rounded-lg text-xs hover:bg-slate-700">
                    ফিল্টার সরান
                  </button>
                </div>
                      ...
                      {pendingTransactionsList.map((tx, idx) => (
                        <tr key={idx} className="hover:bg-slate-800/40">
                          ...
                          <td className="p-3 text-center">
                            {tx.students?.phone ? (
                              <a href={`tel:${tx.students.phone}`} className="bg-emerald-600/20 text-emerald-400 px-3 py-1.5 rounded-lg font-bold hover:bg-emerald-600/40 inline-flex items-center gap-1.5 transition">
