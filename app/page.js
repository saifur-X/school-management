...
export default function Dashboard() {
  ...
  const pendingTransactionsList = students.map(st => {
    ...
    if (due > 0) return { id: st.id, students: st, fee_type: 'Tuition Fee (Pending)', pending_amount: due };
    return null;
  }).filter(Boolean);
            ...
            {!showOnlyPendingList ? (
              ...
              <div className="bg-slate-900/60 border border-slate-800 p-6 rounded-2xl space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <button onClick={() => setShowOnlyPendingList(false)} className="bg-slate-800 px-3 py-1.5 rounded-lg text-xs">ফিল্টার সরান</button>
                </div>
                <div className="overflow-x-auto rounded-xl border border-slate-800">
                  <table className="w-full text-left text-xs border-collapse">
