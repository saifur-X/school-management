...
export default function Dashboard() {
  ...
  const getMonthlyStatus = (monthIndex, studentObj = null, txData = null, cConfig = null) => {
    ...
    if (monthIndex < startM) return { label: 'N/A', bg: 'bg-slate-900/60', text: 'text-slate-600', border: 'border-slate-800/80', badgeBg: 'bg-slate-800/50' };
    ...
    return { label: 'Upcoming', bg: 'bg-slate-800/40', text: 'text-slate-400', border: 'border-slate-700/60', badgeBg: 'bg-slate-700/50' };
  };
  ...
  const sharedGlobalStyle = (
    <style dangerouslySetInnerHTML={{ __html: `
      ...
      .animated-mesh-bg {
        background: linear-gradient(-45deg, #050811, #0d1226, #091a2f, #150c26, #061524, #100b21) !important;
        background-size: 300% 300% !important;
        animation: dynamicMeshAnimation 18s ease infinite !important;
      }
      ...
      .orb-float-anim-1 { animation: floatGlowOrb 14s ease-in-out infinite; }
      .orb-float-anim-2 { animation: floatGlowOrb 18s ease-in-out infinite reverse; }
      .orb-float-anim-3 { animation: floatGlowOrb 16s ease-in-out infinite 2s; }

      /* কার্ড ও বাটন হোভার ইন্টারঅ্যাকশন */
      .card-premium-hover {
        transition: all 0.35s cubic-bezier(0.16, 1, 0.3, 1);
      }
      .card-premium-hover:hover {
        transform: translateY(-3px);
        box-shadow: 0 20px 35px -10px rgba(0, 0, 0, 0.5), 0 0 25px -5px rgba(99, 102, 241, 0.15);
      }

      .btn-glow-blue {
        box-shadow: 0 0 16px -2px rgba(59, 130, 246, 0.45);
        transition: all 0.3s cubic-bezier(0.16, 1, 0.3, 1);
      }
      .btn-glow-blue:hover {
        box-shadow: 0 0 28px 2px rgba(59, 130, 246, 0.7);
      ...
      .btn-glow-emerald:hover {
        box-shadow: 0 0 28px 2px rgba(16, 185, 129, 0.7);
        transform: translateY(-2px);
      }
      .btn-glow-amber {
      ...
      .btn-glow-amber:hover {
        box-shadow: 0 0 28px 2px rgba(245, 158, 11, 0.7);
        transform: translateY(-2px);
      }
      .btn-glow-purple {
      ...
      .btn-glow-purple:hover {
        box-shadow: 0 0 28px 2px rgba(168, 85, 247, 0.7);
        transform: translateY(-2px);
      }
      .btn-glow-rose {
      ...
      .btn-glow-rose:hover {
        box-shadow: 0 0 28px 2px rgba(244, 63, 94, 0.7);
        transform: translateY(-2px);
      }

      .school-brand-title {
        font-family: system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;
        background: linear-gradient(135deg, #60a5fa 0%, #c084fc 45%, #f472b6 75%, #fbbf24 100%);
  ...
  if (studentSession) {
    ...
    return (
      <div className="min-h-screen bg-slate-950 animated-mesh-bg text-slate-100 font-sans p-6 md:p-10 relative overflow-hidden">
        {sharedGlobalStyle}
        ...
        <div className="max-w-6xl mx-auto space-y-8 relative z-10">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-slate-900/80 border border-white/10 p-6 md:p-8 rounded-3xl shadow-2xl backdrop-blur-2xl gap-5">
            <div className="flex items-center gap-5">
              {school.logo_url ? (
                <img src={school.logo_url} alt="Logo" className="w-16 h-16 rounded-2xl bg-white object-contain p-1 border-2 border-indigo-500 shadow-[0_0_25px_rgba(99,102,241,0.5)] shrink-0" />
              ) : (
              ...
              <div>
                <h1 className="text-2xl md:text-3xl font-black tracking-tight school-brand-title leading-tight">
                ...
                <div className="flex flex-wrap items-center gap-3 mt-2 text-xs text-slate-300 font-medium">
                  {school.address && (
                    <span className="flex items-center gap-1.5 bg-slate-950/60 px-3 py-1 rounded-full border border-white/5">
                      <MapPin size={13} className="text-cyan-400 shrink-0"/> {school.address}
                    </span>
                  )}
                  {school.phone && (
                    <span className="flex items-center gap-1.5 bg-slate-950/60 px-3 py-1 rounded-full border border-white/5">
                      <PhoneCall size={12} className="text-emerald-400 shrink-0"/> {school.phone}
                    </span>
                  )}
                  {school.email && (
                    <span className="flex items-center gap-1.5 bg-slate-950/60 px-3 py-1 rounded-full border border-white/5">
                      <Mail size={12} className="text-amber-400 shrink-0"/> {school.email}
                    </span>
                  )}
                </div>
              </div>
            ...
            <div className="flex items-center gap-3 w-full md:w-auto justify-between md:justify-end">
              <span className="px-3.5 py-1.5 rounded-full bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 text-xs font-bold uppercase tracking-wider flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-indigo-400 live-pulse-dot"></span>
                Student Portal
              </span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div className="card-premium-hover bg-gradient-to-br from-blue-900/80 to-indigo-900/80 p-6 rounded-3xl shadow-xl border border-blue-500/40 flex items-center gap-5 backdrop-blur-xl">
            ...
            <div className="card-premium-hover bg-slate-900/70 border border-white/10 p-6 rounded-3xl flex flex-col justify-center backdrop-blur-2xl">
            ...
            <div className="card-premium-hover bg-slate-900/70 border border-white/10 p-6 rounded-3xl flex flex-col justify-center backdrop-blur-2xl"><p className="text-xs text-slate-400 uppercase tracking-widest font-bold">Total Fees Paid</p><h3 className="text-4xl font-black text-emerald-400 mt-2">₹{totalPaid}</h3></div>
            <div className="card-premium-hover bg-slate-900/70 border border-white/10 p-6 rounded-3xl flex flex-col justify-center backdrop-blur-2xl"><p className="text-xs text-slate-400 uppercase tracking-widest font-bold">Current Dues</p><h3 className="text-4xl font-black text-rose-400 mt-2">₹{totalDue}</h3></div>
          </div>
          ...
          <div className="bg-slate-900/70 border border-white/10 p-6 rounded-3xl shadow-xl space-y-4 backdrop-blur-2xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {studentNotices.length > 0 ? studentNotices.map((n) => (
                <div key={n.id} className="card-premium-hover bg-slate-950/80 p-5 rounded-2xl border border-white/10 flex flex-col justify-between space-y-3 hover:border-amber-500/40 transition">
                  <div>
              ...
              {studentPortalData.marks.length > 0 ? (
                <div className="space-y-4">
                  {studentPortalData.marks.map((mRecord, i) => (
                    <div key={i} className="card-premium-hover bg-slate-950 border border-white/10 p-4 rounded-2xl">
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-slate-950 animated-mesh-bg text-slate-100 font-sans relative overflow-hidden">
      {sharedGlobalStyle}
      ...
      {isAdmissionModalOpen && (
        <div className="fixed inset-0 bg-slate-950/85 backdrop-blur-xl z-[60] flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-slate-900/90 border border-white/15 p-6 md:p-8 rounded-3xl w-full max-w-4xl shadow-2xl relative my-8 backdrop-blur-2xl">
            <button onClick={resetStudentForm} className="absolute top-4 right-4 text-slate-400 hover:text-white bg-slate-800/80 p-2 rounded-xl transition hover:bg-slate-700"><X size={20}/></button>
            ...
            <form onSubmit={handleSaveStudent} className="space-y-6">
              <div>
                ...
                <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                  {classList.map(cls => (
                    <button type="button" key={cls} onClick={() => handleClassChangeInForm({target: {value: cls}})} className={`px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap transition ${formData.studentClass === cls ? 'btn-glow-blue bg-blue-600 text-white shadow-md' : 'bg-slate-950 text-slate-400 border border-slate-800 hover:border-slate-700'}`}>{cls}</button>
                  ))}
      ...
      {!teacherSession && isStaffModalOpen && (
        <div className="fixed inset-0 bg-slate-950/85 backdrop-blur-xl z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900/90 border border-white/15 p-6 md:p-8 rounded-3xl max-w-md w-full shadow-2xl relative backdrop-blur-2xl">
      ...
      {isNoticeModalOpen && (
        <div className="fixed inset-0 bg-slate-950/85 backdrop-blur-xl z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900/90 border border-white/15 p-6 md:p-8 rounded-3xl max-w-lg w-full shadow-2xl relative backdrop-blur-2xl">
      ...
      {/* SIDEBAR NAVIGATION */}
      <aside className="w-full md:w-80 bg-slate-900/80 backdrop-blur-2xl border-r border-white/10 p-6 flex flex-col justify-between relative z-20 shadow-2xl">
        <div>
          {/* স্কুল প্রোফাইল ব্রান্ডিং কার্ড */}
          <div className="card-premium-hover p-4 rounded-3xl bg-slate-950/70 border border-white/10 mb-6 shadow-inner backdrop-blur-md">
            <div className="flex items-center gap-3.5 mb-3">
              ...
              <div className="overflow-hidden">
                ...
                <p className="text-[11px] text-indigo-300 font-bold uppercase tracking-wider flex items-center gap-1.5 mt-0.5">
          ...
          <nav className="space-y-2 max-h-[50vh] overflow-y-auto scrollbar-hide pr-1">
            {navItems.map((item) => {
              ...
              return (
                <button 
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-bold transition-all duration-300 ${ activeTab === item.id ? 'btn-glow-blue bg-gradient-to-r from-blue-600 via-indigo-600 to-cyan-600 text-white shadow-lg' : 'text-slate-400 hover:bg-slate-800/60 hover:text-white'}`}
                >
                  <Icon size={18} />{item.label}
                </button>
              );
            })}
            <button onClick={() => router.push('/mark-entry')} className="btn-glow-emerald w-full flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-bold text-emerald-300 bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/30 transition mt-3">
      ...
      <main className="flex-1 p-6 md:p-10 overflow-y-auto w-full relative z-10">
        ...
        {/* টপ ব্যানার */}
        <div className="card-premium-hover bg-slate-900/80 border border-white/10 p-5 md:p-6 rounded-3xl mb-8 shadow-2xl backdrop-blur-2xl flex flex-col lg:flex-row justify-between items-start lg:items-center gap-5">
          <div className="space-y-1.5">
            <h1 className="text-2xl md:text-3xl font-black school-brand-title tracking-tight">
              {school.school_name || 'Smart Educational Institution'}
            </h1>
            <div className="flex flex-wrap items-center gap-3.5 text-xs text-slate-300 font-medium">
              <span className="flex items-center gap-1.5 bg-slate-950/40 px-3 py-1 rounded-full border border-white/5"><MapPin size={13} className="text-cyan-400"/> {school.address || 'Location Address'}</span>
              <span className="flex items-center gap-1.5 font-mono text-emerald-300 bg-slate-950/40 px-3 py-1 rounded-full border border-white/5"><PhoneCall size={13} className="text-emerald-400"/> {school.phone || 'Phone'}</span>
              <span className="flex items-center gap-1.5 text-amber-300 bg-slate-950/40 px-3 py-1 rounded-full border border-white/5"><Mail size={13} className="text-amber-400"/> {school.email || 'Email'}</span>
            </div>
          </div>

          <div className="inline-flex items-center gap-2.5 px-4 py-2.5 rounded-2xl border border-purple-500/40 bg-purple-500/15 text-purple-200 text-xs font-mono font-bold shadow-[0_0_20px_rgba(168,85,247,0.3)] backdrop-blur-md shrink-0">
          ...
          <div className="space-y-8 animate-fade-in">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-white/10 pb-4 gap-4">
               ...
               <div className="flex gap-3 flex-wrap">
                 <button onClick={() => setIsNoticeModalOpen(true)} className="btn-glow-amber bg-gradient-to-r from-amber-600 via-orange-600 to-amber-500 px-5 py-2.5 rounded-2xl font-bold text-white flex items-center gap-2 text-xs">
                   <Megaphone size={16}/> Publish Notice
                 </button>
                 <button onClick={() => setIsAdmissionModalOpen(true)} className="btn-glow-blue bg-gradient-to-r from-blue-600 via-indigo-600 to-cyan-600 px-5 py-2.5 rounded-2xl font-bold text-white flex items-center gap-2 text-xs">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="card-premium-hover bg-slate-900/70 border border-white/10 p-6 rounded-3xl flex items-center gap-4 hover:border-blue-500/40 backdrop-blur-xl">
                <div className="p-4 bg-blue-500/15 text-blue-400 rounded-2xl shadow-[0_0_20px_rgba(59,130,246,0.3)]"><Users size={28} /></div>
              ...
              <div className="card-premium-hover bg-slate-900/70 border border-white/10 p-6 rounded-3xl flex items-center gap-4 hover:border-purple-500/40 backdrop-blur-xl">
                <div className="p-4 bg-purple-500/15 text-purple-400 rounded-2xl shadow-[0_0_20px_rgba(168,85,247,0.3)]"><Briefcase size={28} /></div>
              ...
              <div className="card-premium-hover bg-slate-900/70 border border-white/10 p-6 rounded-3xl flex items-center gap-4 hover:border-amber-500/40 backdrop-blur-xl">
                <div className="p-4 bg-amber-500/15 text-amber-400 rounded-2xl shadow-[0_0_20px_rgba(245,158,11,0.3)]"><Megaphone size={28} /></div>
            ...
            <div className="bg-slate-900/70 border border-white/10 p-6 md:p-8 rounded-3xl shadow-2xl space-y-5 backdrop-blur-2xl">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                {notices.length > 0 ? notices.map((n) => (
                  <div key={n.id} className="card-premium-hover bg-slate-950/80 p-5 rounded-2xl border border-white/10 flex flex-col justify-between space-y-3 hover:border-amber-500/40 backdrop-blur-md">
                    <div>
                      <div className="flex justify-between items-start gap-2">
                        <h4 className="font-bold text-white text-base leading-snug">{n.title}</h4>
                        <span className="bg-slate-800/80 text-slate-300 px-2.5 py-0.5 rounded-md text-[10px] font-bold whitespace-nowrap border border-white/5">{n.publish_date}</span>
                      </div>
          ...
          <div className="space-y-6 animate-fade-in w-full">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-white/10 pb-5 gap-4">
              <div>
                ...
                <p className="text-xs text-slate-300 mt-1">বারকোড স্ক্যানার, ডে-টাইপ সিলেকশন ও লাইভ উপস্থিতি মনিটরিং।</p>
              </div>

              <div className="flex flex-wrap gap-2.5">
                <div className="bg-slate-900/80 border border-white/10 p-1.5 rounded-2xl flex gap-1 backdrop-blur-xl shadow-inner">
                  <button 
            ...
            <div className="card-premium-hover bg-slate-900/70 border border-white/10 p-4 rounded-3xl shadow-xl backdrop-blur-2xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              <div className="flex items-center gap-3">
                <div className="p-2.5 rounded-xl bg-amber-500/10 border border-amber-500/20">
                  <Sun size={20} className="text-amber-400"/>
                </div>
                <div>
                  <h4 className="text-xs font-bold uppercase tracking-wider text-slate-300">Select Day Schedule:</h4>
                  <p className="text-[11px] text-slate-400">তারিখ: <span className="text-white font-mono font-bold">{attendanceDate}</span> ({dayType})</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 w-full md:w-auto">
                <button
                  type="button"
                  onClick={() => handleSelectDayType('Class Day')}
                  className={`px-4 py-2 rounded-2xl text-xs font-bold transition flex items-center justify-center gap-1.5 ${dayType === 'Class Day' ? 'btn-glow-emerald bg-emerald-600 text-white' : 'bg-slate-950/80 text-slate-400 border border-white/10 hover:border-emerald-500'}`}
                >
                ...
                <button
                  type="button"
                  onClick={() => handleSelectDayType('Exam Day')}
                  className={`px-4 py-2 rounded-2xl text-xs font-bold transition flex items-center justify-center gap-1.5 ${dayType === 'Exam Day' ? 'btn-glow-amber bg-amber-600 text-white' : 'bg-slate-950/80 text-slate-400 border border-white/10 hover:border-amber-500'}`}
                >
                ...
                <button
                  type="button"
                  onClick={() => handleSelectDayType('Class Off')}
                  className={`px-4 py-2 rounded-2xl text-xs font-bold transition flex items-center justify-center gap-1.5 ${dayType === 'Class Off' ? 'bg-orange-600 text-white shadow-lg shadow-orange-600/30' : 'bg-slate-950/80 text-slate-400 border border-white/10 hover:border-orange-500'}`}
                >
                ...
                <button
                  type="button"
                  onClick={() => handleSelectDayType('Holiday')}
                  className={`px-4 py-2 rounded-2xl text-xs font-bold transition flex items-center justify-center gap-1.5 ${dayType === 'Holiday' ? 'btn-glow-rose bg-rose-600 text-white' : 'bg-slate-950/80 text-slate-400 border border-white/10 hover:border-rose-500'}`}
                >
            ...
            {(dayType === 'Holiday' || dayType === 'Class Off') && (
              <div className="p-4 rounded-2xl bg-gradient-to-r from-rose-950/40 via-orange-950/40 to-slate-900/50 border border-rose-500/30 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 backdrop-blur-xl">
                ...
                <button
                  onClick={handleMarkAllAsLeave}
                  className="px-4 py-1.5 rounded-xl bg-rose-600 hover:bg-rose-500 text-white text-xs font-bold transition shrink-0"
                >
            ...
            {attendanceView === 'daily' && (
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3.5">
                <div className="card-premium-hover bg-slate-900/70 border border-white/10 p-4 rounded-2xl backdrop-blur-xl">
                  <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Total Active</p>
                  <h4 className="text-2xl font-black text-white mt-1">{currentAttendanceTargetList.length}</h4>
                </div>
                <div className="card-premium-hover bg-emerald-950/30 border border-emerald-500/30 p-4 rounded-2xl backdrop-blur-xl">
                  <p className="text-[10px] text-emerald-400 font-bold uppercase tracking-wider">Present</p>
                  <h4 className="text-2xl font-black text-emerald-400 mt-1">{presentCount}</h4>
                </div>
                <div className="card-premium-hover bg-rose-950/30 border border-rose-500/30 p-4 rounded-2xl backdrop-blur-xl">
                ...
                <div className="card-premium-hover bg-amber-950/30 border border-amber-500/30 p-4 rounded-2xl backdrop-blur-xl">
                ...
                <div className="card-premium-hover bg-blue-950/30 border border-blue-500/30 p-4 rounded-2xl backdrop-blur-xl">
                ...
                <div className="card-premium-hover bg-indigo-950/40 border border-indigo-500/40 p-4 rounded-2xl backdrop-blur-xl flex flex-col justify-center">
            ...
            {/* কন্ট্রোল বার */}
            <div className="card-premium-hover bg-slate-900/70 border border-white/10 p-5 md:p-6 rounded-3xl backdrop-blur-2xl shadow-xl flex flex-col lg:flex-row gap-5 justify-between items-stretch lg:items-center">
                ...
                {attendanceMode === 'students' && (
                  <div>
                    ...
                    <select 
                      ...
                      className="bg-slate-950 border border-white/10 p-3 rounded-2xl text-white text-xs font-bold outline-none focus:border-emerald-500 shadow-inner"
                    >
              ...
              {attendanceView === 'daily' && (
                <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center">
                  ...
                  <button 
                    ...
                    className="btn-glow-emerald bg-gradient-to-r from-emerald-600 to-teal-500 text-white px-5 py-3 rounded-2xl text-xs font-bold flex items-center justify-center gap-2 whitespace-nowrap shadow-lg"
                  >
              ...
              <div className="bg-slate-900/70 border border-white/10 rounded-3xl overflow-hidden shadow-2xl backdrop-blur-2xl">
                <div className="overflow-x-auto">
                  <table className="w-full text-left min-w-[700px]">
                    <thead className="bg-slate-800/70 text-slate-300 text-xs uppercase tracking-wider border-b border-white/10">
                      <tr>
                    ...
                    <tbody className="divide-y divide-white/5 text-sm">
                      {currentAttendanceTargetList.map(person => {
                        ...
                        return (
                          <tr key={stId} className="hover:bg-white/5 transition duration-150">
                            <td className="p-4">
                            ...
                            <td className="p-4 text-center">
                              <div className="inline-flex bg-slate-950/90 p-1.5 rounded-2xl border border-white/10 gap-1.5 shadow-inner">
                                <button 
                            ...
                            <td className="p-4 text-right">
                              {attendanceMode === 'students' && isAbsent && waLink ? (
                                <a 
                                  ...
                                  className="btn-glow-emerald bg-emerald-600/20 text-emerald-300 border border-emerald-500/40 hover:bg-emerald-600 hover:text-white px-3.5 py-1.5 rounded-xl text-xs font-bold inline-flex items-center gap-1.5 transition shadow-sm"
                                >
                      ...
                      {currentAttendanceTargetList.length === 0 && (
                        <tr><td colSpan="4" className="text-center py-10 text-slate-500 font-semibold">এই ক্লাসে কোনো স্টুডেন্ট খুঁজে পাওয়া যায়নি। নিশ্চিত করুন যে স্টুডেন্ট অ্যাডমিশন হয়েছে।</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
        ...
        {activeTab === 'students' && (
          <div className="space-y-6 animate-fade-in w-full">
            ...
            <div className="flex bg-slate-950/80 border border-white/10 p-1.5 rounded-2xl w-fit backdrop-blur-md shadow-inner">
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-4">
              <div className="relative w-full md:flex-1">
                ...
                <input type="text" placeholder="Search by name, roll or Unique ID..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-slate-900/80 border border-white/10 pl-12 pr-4 py-3 rounded-2xl text-sm text-white focus:outline-none focus:ring-2 focus:ring-blue-500 backdrop-blur-md shadow-inner" />
              </div>
              ...
              <div className="flex gap-3 w-full md:w-auto overflow-x-auto scrollbar-hide pb-2 md:pb-0">
                ...
                <select onChange={(e) => setSelectedClassFilter(e.target.value)} value={selectedClassFilter} className="bg-slate-900 text-white border border-white/10 px-4 py-2.5 rounded-2xl text-xs font-bold outline-none shadow-inner">
        ...
        {!teacherSession && activeTab === 'class_mgmt' && (
          <div className="space-y-6 animate-fade-in w-full">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="card-premium-hover bg-slate-900/70 border border-white/10 p-6 rounded-3xl space-y-4 backdrop-blur-2xl">
            ...
            <button onClick={handleSaveClassConfig} disabled={isProcessing} className="btn-glow-emerald bg-gradient-to-r from-emerald-600 to-teal-500 px-8 py-3.5 rounded-2xl font-black transition flex items-center gap-2 shadow-lg">
        ...
        {!teacherSession && activeTab === 'erp' && (
          <div className="space-y-6 animate-fade-in w-full">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="card-premium-hover bg-slate-900/70 border border-white/10 p-6 rounded-3xl flex items-center justify-between shadow-2xl backdrop-blur-2xl">
                <div className="flex items-center gap-4">
                  <div className="p-4 bg-emerald-500/15 text-emerald-400 rounded-2xl shadow-[0_0_20px_rgba(16,185,129,0.3)]"><DollarSign size={32} /></div>
                  <div>
              ...
              <div className="card-premium-hover bg-slate-900/70 border border-white/10 p-6 rounded-3xl flex items-center justify-between shadow-2xl backdrop-blur-2xl">
                <div className="flex items-center gap-4">
                  <div className="p-4 bg-rose-500/15 text-rose-400 rounded-2xl shadow-[0_0_20px_rgba(244,63,94,0.3)]"><AlertCircle size={32} /></div>
                  <div>
                      ...
                      <div>
                        ...
                        <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                          {activeClasses.length > 0 ? activeClasses.map(c => (
                            <button key={c} onClick={() => { setErpSelectedClass(c); setErpStudent(null); }} className={`px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap transition ${erpSelectedClass === c ? 'btn-glow-blue bg-blue-600 text-white shadow-md' : 'bg-slate-950 text-slate-400 border border-slate-800 hover:bg-slate-800'}`}>
                              {c}
                            </button>
                  ...
                  <div className="lg:col-span-8 space-y-6">
                    <div className="bg-slate-900/70 border border-white/10 p-6 rounded-3xl shadow-xl overflow-x-auto backdrop-blur-2xl">
                      <div className="flex flex-wrap justify-between items-center mb-4 gap-2">
                        <h3 className="text-lg font-bold text-blue-400 flex items-center gap-2"><Calendar size={18}/> Status Tracker ({erpClassConfig.academic_year})</h3>
                        {(() => {
                    ...
                    <div className="bg-slate-900/70 border border-white/10 p-6 rounded-3xl shadow-xl space-y-6 backdrop-blur-2xl">
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {feeOptionsList.map(fee => (
                            <button type="button" key={fee} onClick={() => toggleFeeType(fee)} className={`px-4 py-2.5 rounded-2xl text-xs font-bold border transition ${ erpSelectedFeeTypes.includes(fee) ? 'btn-glow-blue bg-blue-600 text-white border-blue-500 shadow-md' : 'bg-slate-950 text-slate-400 border-slate-800 hover:border-slate-600 hover:text-white' }`}>
                              {fee}
                            </button>
                          ))}
                        </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-slate-950/80 p-5 rounded-2xl border border-white/10 shadow-inner">
                        <div>
                        ...
                        <div>
                          ...
                          <input type="number" placeholder="Enter amount" value={erpDiscount} onChange={(e) => setErpDiscount(e.target.value)} className="w-full bg-slate-900 border border-slate-700 p-3 rounded-xl text-white text-sm outline-none focus:border-blue-500 shadow-inner" />
                        </div>
                      </div>
                      ...
                      <button onClick={handleCreateInvoice} disabled={isProcessing} className="btn-glow-emerald w-full bg-gradient-to-r from-emerald-600 to-teal-500 text-white py-4 rounded-2xl font-black text-sm transition flex justify-center items-center gap-2 shadow-lg">
          ...
          <div className="space-y-6 animate-fade-in w-full">
            ...
            <div className="bg-slate-900/70 border border-white/10 p-6 rounded-3xl shadow-xl space-y-6 backdrop-blur-2xl">
              <div>
                ...
                <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                  {activeClasses.length > 0 ? activeClasses.map(c => (
                    <button key={c} onClick={() => { setIdSelectedClass(c); setSelectedIdStudent(null); }} className={`px-5 py-2.5 rounded-2xl text-xs font-bold whitespace-nowrap transition ${idSelectedClass === c ? 'btn-glow-blue bg-blue-600 text-white shadow-md' : 'bg-slate-950 text-slate-400 border border-slate-800 hover:bg-slate-800'}`}>
                      {c}
                    </button>
        ...
        {!teacherSession && activeTab === 'profile' && (
          <div className="space-y-6 animate-fade-in w-full pb-10">
            ...
            {editSchool ? (
              ...
              <div className="card-premium-hover bg-slate-900/70 border border-white/10 rounded-3xl shadow-2xl overflow-hidden relative backdrop-blur-2xl">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 p-6 md:p-10">
                  <div className="card-premium-hover bg-slate-950/70 border border-white/10 p-6 rounded-2xl hover:border-white/20 transition">
                  ...
                  <div className="card-premium-hover bg-slate-950/70 border border-white/10 p-6 rounded-2xl hover:border-white/20 transition flex flex-col">
